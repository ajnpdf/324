...
const authSetup = read('scripts/R23_AUTH_SETUP.ps1');
...
check('Firebase auth covers email signup/login/reset, Google, Facebook, GitHub and token refresh', ['accounts:signUp','accounts:signInWithPassword','accounts:sendOobCode','securetoken.googleapis.com','GoogleAuthProvider','FacebookAuthProvider','GithubAuthProvider','signInWithPopup'].every((value) => firebaseRest.includes(value)));
check('authentication UI exposes Google, Facebook, GitHub and Email/Gmail only', ['Google','Facebook','GitHub','email / Gmail'].every((value) => authUi.includes(value)) && !firebaseRest.includes('signInAnonymously') && !authUi.includes('Continue anonymously'));
check('authentication setup disables anonymous and configures production OAuth providers', authSetup.includes('anonymous = @{ enabled = $false }') && authSetup.includes("Set-IdpConfig 'facebook.com'") && authSetup.includes("Set-IdpConfig 'github.com'") && authSetup.includes("Enable-ExistingIdp 'google.com'") && authSetup.includes('authorizedDomains'));
...
check('Firebase and Windows/Android setup assets exist', ['firebase.json','firestore.rules','firestore.indexes.json','.env.r21.example','scripts/R21_FIREBASE_SETUP.ps1','scripts/R23_AUTH_SETUP.ps1','scripts/R21_CREATE_API_KEY.ps1','scripts/R21_BUILD_ANDROID.ps1','scripts/R21_SETUP_ALL.ps1','scripts/R21_FINAL_PRODUCTION.ps1'].every(exists));
