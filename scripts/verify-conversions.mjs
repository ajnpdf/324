...
if (ids.length >= 70) pass(`${ids.length} conversion and OCR tools registered`); else fail(`Expected at least 70 conversion tools, found ${ids.length}`);
...
for (const expected of [
  ...
  'docx-to-pdf', 'xlsx-to-pdf', 'pptx-to-pdf', 'ods-to-pdf', 'odp-to-pdf', 'eml-to-pdf', 'msg-to-pdf', 'xps-to-pdf',
]) frontend.includes(`'${expected}'`) ? pass(`Required tool ${expected}`) : fail(`Missing required tool ${expected}`);
...
component.includes('language') && component.includes('dpi') && component.includes('quality') ? pass('OCR and image conversion options enabled') : fail('Conversion options incomplete');
...
backend.includes('available_ocr_languages') && component.includes('ocrLanguages') ? pass('OCR language choices follow installed backend language data') : fail('OCR language capability discovery is missing');
backend.includes('ocr-source-') && backend.includes('ocr-page-') && backend.includes('merged.insert_pdf') ? pass('Searchable PDF OCR uses project work files and deterministic merging') : fail('Searchable PDF OCR still relies on fragile temporary output handling');
!backend.includes('image_to_pdf_or_hocr') ? pass('Fragile pytesseract temporary PDF helper removed') : fail('Fragile pytesseract temporary PDF helper is still present');
backend.includes('tessedit_create_pdf=1') && !backend.includes('\"pdf\",\n                    ],') ? pass('Searchable PDF does not require the optional Tesseract pdf config file') : fail('Searchable PDF still depends on a Tesseract pdf config file');
