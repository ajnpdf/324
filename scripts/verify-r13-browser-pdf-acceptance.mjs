...
if (!fs.existsSync(sourcePath)) {
  console.error('FAIL: src/lib/pdf-manipulator.ts is missing.');
  process.exit(1);
}
...
try {
  ...
  const mergeResult = await new PDFManipulator(mergeQueue).runOperation('merge-pdf', { outputName: 'r13-merged' });
  const merged = await loadResult(mergeResult);
  expect(JSON.stringify(widths(merged)) === JSON.stringify([403,401,405,402,404]), `merge order mismatch: ${widths(merged)}`);
  pass('Merge PDF preserves the selected file order across five real generated PDFs');

  const afterRemoveAndReAdd = [files[2], files[0], files[1], files[3], files[4]];
  const secondMerge = await loadResult(await new PDFManipulator(afterRemoveAndReAdd).runOperation('merge-pdf', { outputName: 'r13-reordered' }));
  ...
  pass('Merge PDF output matches a reorder/remove/re-add queue');
