...
const securityIds = ['protect-pdf','unlock-pdf','repair-pdf'];
const publicRouteIds = [...new Set([...stable,...visibleLimited,...conversionIds,...securityIds])].filter((id)=>!policyLegacy.includes(id)).sort();
const canonicalIds = publicRouteIds.filter((id)=>!workspaceAliases.has(id)).sort();
const processing=(id)=>conversionIds.includes(id)||securityIds.includes(id)?'temporary-server':'browser';
const releaseClass=(id)=>conversionIds.includes(id)?'conversion':id==='sign-pdf'?'signature':securityIds.includes(id)?'security':'utility';
const canonicalTools = canonicalIds.map((id)=>({id,route:`/${id}`,processing:processing(id),releaseClass:releaseClass(id)}));
const publicRoutes = publicRouteIds.map((id)=>({id,route:`/${id}`,canonicalId:workspaceAliases.get(id)||id,alias:workspaceAliases.has(id),processing:processing(workspaceAliases.get(id)||id)}));
fs.mkdirSync(path.join(root,'reports'),{recursive:true});
const output={generatedAt:new Date().toISOString(),publicRouteCount:publicRoutes.length,canonicalProcessorCount:canonicalTools.length,policyLegacyAliasesHidden:policyLegacy.sort(),publicWorkspaceAliases:[...workspaceAliases.entries()].filter(([alias])=>publicRouteIds.includes(alias)).map(([alias,canonicalId])=>({alias,canonicalId})).sort((a,b)=>a.alias.localeCompare(b.alias)),publicRoutes,canonicalTools};
