const QUALITY_LIMITATIONS: Record<string, string> = {
  ...
  'scanned-pdf-to-word': 'Creates editable OCR text from scanned pages. Recognition and layout fidelity depend on scan quality and the selected language.',
  'scanned-pdf-to-searchable-pdf': 'Keeps the scanned page appearance while adding an OCR text layer. Recognition should be reviewed for important documents.',
  'image-to-searchable-pdf': 'Keeps the image appearance while adding an OCR text layer. Recognition quality depends on source clarity and language selection.',
