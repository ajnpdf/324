...
ctx.onmessage = async (e: MessageEvent) => {
  ...
  try {
    ...
    for (let i = 0; i < total; i++) {
      if (i % 20 === 0) {
        ctx.postMessage({ type: 'progress', value: Math.round((i / total) * 85), stage: `Analyzing binary node ${i + 1}...` });
      }
      ...
      if (settings.recompressImages) {
        ...
        try {
          ...
          console.warn("[Worker] Image recompression failed at node:", ref.toString());
        }
      }
    }

    if (settings.removeFormFields) {
      try { pdfDoc.getForm().flatten(); } catch (fErr) { console.warn("[Worker] Form flattening skipped."); }
    }

    ctx.postMessage({ type: 'progress', value: 95, stage: "Finalizing binary stream..." });
    ...
    ctx.postMessage({ type: 'error', error: err.message || "Binary synthesis interrupt." });
  }
};