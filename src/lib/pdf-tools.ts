...
export async function mergePDFs(files: File[], onProgress: (p: number, s: string) => void) {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    onProgress(stepProgress, `Ingesting Binary: ${file.name}`);
...
export async function splitPDFByRange(file: File, rangeStr: string, onProgress: (p: number, s: string) => void) {
  ...
  onProgress(50, "Extracting Binary Range...");
 ...
 */
export async function compressPDF(file: File, level: 'low' | 'medium' | 'high', onProgress: (p: number, s: string) => void) {
  onProgress(20, "Initiating Stream Deflation...");
  const arrayBuffer = await file.arrayBuffer();
  const pdfDoc = await PDFDocument.load(arrayBuffer.slice(0));
  
  onProgress(60, "Optimizing Internal Dictionaries...");
