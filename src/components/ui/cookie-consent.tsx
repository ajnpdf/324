    ...
    <AnimatePresence>
      {show && (
        <motion.div initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 28 }} className="fixed inset-x-0 bottom-0 z-[250] p-3 md:p-5">
          <div className="mx-auto max-w-5xl rounded-3xl border border-slate-200 bg-white/96 p-5 dark:border-slate-800 dark:bg-slate-950/96 shadow-[0_-24px_70px_rgba(15,23,42,.16)] backdrop-blur-2xl md:p-6">
            <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
              <div className="flex gap-4">
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-blue-50 text-blue-600 dark:bg-blue-500/15 dark:text-blue-300"><Cookie className="h-5 w-5" /></div>
                <div>
                  <h2 className="text-sm font-black text-slate-950 dark:text-slate-50">{t('cookie.title')}</h2>
                  <p className="mt-1 max-w-2xl text-xs font-medium leading-5 text-slate-600 dark:text-slate-300">{t('cookie.body')}</p>
              ...
              <div className="flex shrink-0 flex-col gap-2 sm:flex-row">
                <Button type="button" variant="outline" onClick={() => save('declined')} data-analytics-id="cookie-decline" className="h-11 rounded-xl border-slate-200 px-5 dark:border-slate-700 text-xs font-black">{t('cookie.decline')}</Button>
