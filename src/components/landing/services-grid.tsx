...
const GROUPS = [
  {
    ...
    description: 'The everyday tools for assembling, organizing and preparing PDF files.',
    ids: ['merge-pdf','split-pdf','compress-pdf','rotate-pdf','delete-pdf-pages','organize-pdf','crop-pdf','watermark-pdf','page-number','flatten-pdf'],
  },
  {
    id: 'edit',
    title: 'Edit & Sign',
    description: 'Add content, inspect documents, extract assets and finish a PDF for sharing.',
    ids: ['add-text','add-image-to-pdf','compare-pdf','pdf-metadata','extract-images','sign-pdf','pdf-zip-extract'],
  },
  {
    id: 'security',
    title: 'Security & Recovery',
    description: 'Protect authorized documents, remove known passwords and repair damaged PDF structure.',
    ids: ['protect-pdf','unlock-pdf','repair-pdf'],
  },
  {
    id: 'image',
    title: 'Image Tools',
    description: 'Resize, reduce, crop, rotate, watermark, flip and convert common image formats.',
    ids: ['image-reducer','image-resizer','crop-image','rotate-image','watermark-image','flip-image','convert-image'],
  },
] as const;
  ...
  return <Link href={toolPath(tool.id)} prefetch={false} className="group block h-full rounded-2xl focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-600 focus-visible:ring-offset-2" aria-label={localized.name} data-analytics-id={`tool-card-${tool.id}`} data-analytics-category={category}>
    <article className="h-full rounded-2xl border border-slate-200 bg-white transition duration-150 hover:-translate-y-0.5 hover:border-violet-200 hover:shadow-[0_12px_30px_rgba(76,29,149,.08)]">
      <div className="flex min-h-[86px] items-center gap-3 px-3.5 py-3.5">
        ...
        <div className="min-w-0 flex-1"><h3 className="truncate text-[13.5px] font-black leading-5 tracking-[-.01em] text-slate-950 sm:text-[14px]"><Highlight text={localized.name} highlight={query}/></h3><p className="mt-0.5 line-clamp-2 text-[10.5px] font-medium leading-4 text-slate-560 sm:text-[11px] text-slate-600"><Highlight text={localized.desc} highlight={query}/></p></div>
...
export function ServicesGrid({query,category}:ServicesGridProps){
  ...
  const filteredTools=useMemo(()=>{
    ...
    }).filter(x=>matchesCategory(x.tool,category)&&(!normalized||x.score>0)).sort((a,b)=>normalized?(b.score-a.score||a.index-b.index):a.index-b.index).map(x=>x.tool);
  ...
  if(groupedMode){
    ...
    return <div className="space-y-10" id="ajn-public-tool-grid">
      {GROUPS.map((group,groupIndex)=>{
        const tools=group.ids.map(id=>byId.get(id)).filter((tool):tool is PublicTool=>Boolean(tool));
        return <section key={group.id} aria-labelledby={`tool-group-${group.id}`}>
          <div className="mb-4 flex items-end justify-between gap-4 border-b border-slate-100 pb-3">
            <div><h3 id={`tool-group-${group.id}`} className="text-lg font-black tracking-[-.025em] text-slate-950 md:text-xl">{group.title}</h3><p className="mt-1 hidden text-xs font-medium leading-5 text-slate-500 sm:block">{group.description}</p></div>
            <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-black text-slate-500">{tools.length}</span>
          </div>
          <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{tools.map((tool,index)=><ToolCard key={tool.id} tool={tool} query="" priority={groupIndex===0&&index<6}/>)}</div>
        </section>;
      })}
    </div>;
  }
  ...
  return <div className="space-y-4">
    <div className="flex items-center justify-between"><p className="text-[11px] font-bold text-slate-600" aria-live="polite">{filteredTools.length} matching {filteredTools.length===1?'tool':'tools'}</p></div>
    <div id="ajn-public-tool-grid" className="grid grid-cols-1 gap-2.5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{filteredTools.map((tool,index)=><ToolCard key={tool.id} tool={tool} query={query} priority={index<6}/>)}</div>
  </div>;
}
