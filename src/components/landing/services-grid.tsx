...
function Highlight({ text, highlight }: { text: string; highlight: string }) {
  ...
  return <>{parts.map((part,index)=>part.toLowerCase()===q.toLowerCase()?<mark key={index} className="rounded-sm bg-blue-50 px-0.5 text-blue-700">{part}</mark>:<span key={index}>{part}</span>)}</>;
}
...
function ToolCard({ tool, query, priority=false, view }:{tool:(typeof BUILD_PUBLIC_TOOLS)[number];query:string;priority?:boolean;view:ViewMode}){
  ...
  return <Link href={`/tools/${tool.id}`} className="group block h-full rounded-2xl focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2" aria-label={localized.name} data-analytics-id={`tool-card-${tool.id}`} data-analytics-category={category}>
...
export function ServicesGrid({query,category}:ServicesGridProps){
  ...
  return <div className="space-y-4 md:space-y-6"><div className="hidden items-center justify-between gap-4 md:flex"><p className="text-[11px] font-bold text-slate-500">{t('home.toolsCount',{count:filteredTools.length})}</p><div className="flex items-center gap-1 rounded-xl border border-slate-200 bg-white p-1 shadow-sm" role="group" aria-label={t('home.layoutAria')}>{viewOptions.map(({id,label,icon:Icon})=><button key={id} type="button" onClick={()=>chooseView(id)} aria-pressed={view===id} title={label} className={cn('inline-flex min-h-9 items-center gap-1.5 rounded-lg px-2.5 text-[10px] font-black transition focus-visible:outline focus-visible:outline-2 focus-visible:outline-blue-500',view===id?'bg-slate-950 text-white shadow-sm':'text-slate-500 hover:bg-slate-50 hover:text-slate-900')}><Icon className="h-3.5 w-3.5"/><span>{label}</span></button>)}</div></div><div className={cn('grid gap-2.5 sm:gap-3',gridClass)}>{filteredTools.map((tool,index)=><ToolCard key={tool.id} tool={tool} query={query} priority={index<8} view={view}/>)}</div>{filteredTools.length===0&&<div className="ajn-glass-card rounded-2xl py-12 text-center"><Search className="mx-auto h-8 w-8 text-slate-300"/><p className="mt-4 text-base font-black text-slate-800">{t('home.noMatch')}</p><p className="mt-2 text-sm text-slate-500">{t('home.noMatchHint')}</p></div>}</div>;
}
