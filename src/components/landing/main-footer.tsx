...
export function MainFooter() {
  ...
  const trustItems = [t('footer.processingShown'),t('footer.serverLabel'),t('footer.noGuessing'),t('footer.noSignup')];
  return <footer className="relative mt-20 overflow-hidden border-t border-slate-800 bg-slate-950 text-white">
    <div className="relative mx-auto max-w-7xl px-4 py-14 md:px-8 md:py-20">
      <div className="grid gap-10 lg:grid-cols-[1.2fr_.8fr_.8fr_.9fr]">
        <div><LogoAnimation className="h-11 w-[176px] rounded-xl bg-white px-2 py-1 shadow-sm" /><p className="mt-5 max-w-sm text-sm font-medium leading-7 text-slate-300">{t('footer.about')}</p><div className="mt-6 flex gap-2">{socialLinks.map(({icon:Icon,href,label})=><a key={href} href={href} target={href.startsWith('http')?'_blank':undefined} rel={href.startsWith('http')?'me noreferrer':undefined} aria-label={label} className="flex h-11 w-11 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-slate-300 transition hover:border-blue-400/40 hover:bg-blue-500/15 hover:text-white"><Icon className="h-4 w-4"/></a>)}</div></div>
        <div><h3 className="text-xs font-black tracking-[.12em] text-blue-300">{t('footer.commonTools')}</h3><nav className="mt-5 space-y-3">{commonToolIds.map(id=>{const [name,desc]=commonFallback[id];const localized=tool(id,name,desc,[]);return <Link key={id} href={`/tools/${id}`} className="flex items-center gap-2 text-xs font-bold text-slate-300 hover:text-white"><ArrowRight className="h-3 w-3 text-blue-400"/>{localized.name}</Link>})}</nav></div>
