...
const productLinks = [
  ['PDF Tools','/pdf-tools'],['AJN Sign','/sign'],['AJN API','/developers'],['AJN Desktop','/desktop'],['AJN Mobile','/mobile'],['AJN IMG','/img'],['Pricing','/pricing'],['Account','/account'],['About','/about'],['Contact','/contact'],
] as const;
const legalLinks = [
  ['Privacy Policy','/privacy'],['Terms','/terms'],['Cookie Policy','/cookies'],['File Processing Policy','/file-processing-policy'],
  ['Acceptable Use','/acceptable-use'],['Data Deletion','/data-deletion'],['Unlock Authorization','/unlock-authorization-policy'],['Copyright','/copyright'],['DMCA','/dmca']
] as const;
...
export function MainFooter() {
  ...
  const trustItems = ['Clear tool-specific limits','Core tools work without an account','Temporary-server workflows are disclosed','Downloads stay under your control'];
  return <footer className="relative mt-20 overflow-hidden border-t border-slate-800 bg-slate-950 text-white">
    <div className="relative mx-auto max-w-7xl px-4 py-14 md:px-8 md:py-20">
      <div className="grid gap-10 lg:grid-cols-[1.2fr_.8fr_.8fr_.9fr]">
        <div><LogoAnimation className="h-11 w-[176px]" /><p className="mt-5 max-w-sm text-sm font-medium leading-7 text-slate-300">Free online PDF tools for merging, compressing, organizing, editing, signing and protecting documents, connected to the wider AJN product family.</p><div className="mt-6 flex gap-2">{socialLinks.map(({icon:Icon,href,label})=><a key={href} href={href} aria-label={label} className="flex h-11 w-11 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-slate-300 transition hover:border-violet-400/40 hover:bg-violet-500/15 hover:text-white"><Icon className="h-4 w-4"/></a>)}</div></div>
        <div><h3 className="text-xs font-black tracking-[.12em] text-violet-300">PDF TOOLS</h3><nav className="mt-5 space-y-3">{commonToolIds.map(id=>{const [name,desc]=commonFallback[id];const localized=tool(id,name,desc,[]);return <Link key={id} href={toolPath(id)} className="flex items-center gap-2 text-xs font-bold text-slate-300 hover:text-white"><ArrowRight className="h-3 w-3 text-violet-400"/>{localized.name}</Link>})}</nav></div>
        <div><h3 className="text-xs font-black tracking-[.12em] text-emerald-300">AJN PRODUCTS</h3><nav className="mt-5 space-y-3">{productLinks.map(([label,href])=><Link key={href} href={href} className="block text-xs font-bold text-slate-300 hover:text-white">{label}</Link>)}</nav></div>
        <div><h3 className="text-xs font-black tracking-[.12em] text-red-300">LEGAL & TRUST</h3><nav className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-1">{legalLinks.map(([label,href])=><Link key={href} href={href} className="block text-xs font-bold text-slate-300 hover:text-white">{label}</Link>)}</nav><button type="button" onClick={openPrivacyChoices} className="mt-5 inline-flex min-h-11 items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-xs font-black text-white hover:bg-white/10"><Cookie className="h-4 w-4"/>{t('cookie.privacy')}</button></div>
      </div>
