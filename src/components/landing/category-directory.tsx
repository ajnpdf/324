...
const intentContent = {
  conversion:{outcomes:['Convert PDFs to editable document formats','Create PDFs from documents and structured data','Run OCR on scans and document photos'],workflows:[{href:'/tools/pdf-to-docx',label:'PDF to Word'},{href:'/tools/docx-to-pdf',label:'Word to PDF'},{href:'/tools/scanned-pdf-to-text',label:'Scanned PDF to Text'},{href:'/tools/pdf-to-xlsx',label:'PDF to Excel'}]},
  image:{outcomes:['Combine images into a PDF','Export PDF pages into image files','Prepare scans, receipts and searchable documents'],workflows:[{href:'/tools/jpg-to-pdf',label:'JPG to PDF'},{href:'/tools/png-to-pdf',label:'PNG to PDF'},{href:'/tools/pdf-to-png',label:'PDF to PNG'},{href:'/tools/image-to-text',label:'Image to Text OCR'}]},
  pdf:{outcomes:['Combine, split and organize pages','Protect, unlock and repair authorized files','Compress, crop, rotate and prepare documents'],workflows:[{href:'/tools/merge-pdf',label:'Merge PDF'},{href:'/tools/split-pdf',label:'Split PDF'},{href:'/tools/compress-pdf',label:'Compress PDF'},{href:'/tools/protect-pdf',label:'Protect PDF'}]},
} as const;
...
export function CategoryDirectory({ category }: { category: PublicToolCategory }) {
  ...
  <div className="mt-12 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">{tools.map(tool=><Link key={tool.id} href={`/tools/${tool.id}`} className="ajn-tool-card ajn-horizontal-tool-card group block"><div className="relative z-10 flex min-h-[80px] items-center gap-3 p-3"><ToolArtwork toolId={tool.id} toolName={tool.name} className="h-11 w-11"/><div className="min-w-0 flex-1"><div className="flex items-center justify-end"><ArrowRight className="h-4 w-4 text-slate-300 transition group-hover:translate-x-1 group-hover:text-blue-600"/></div><h2 className="-mt-3 line-clamp-1 text-[14px] font-black tracking-tight text-foreground">{tool.name}</h2><p className="mt-1 line-clamp-1 text-[11px] font-medium leading-4 text-muted-foreground">{tool.desc}</p></div></div></Link>)}</div>
