...
const GROUP_ORDER = ['Popular','Organize PDF','Edit & Sign','Security & Recovery','Image Tools','More Tools'] as const;
type GroupName = (typeof GROUP_ORDER)[number];

function groupFor(tool: ServiceTool): GroupName {
  const id = tool.id;
  if (POPULAR_IDS.includes(id)) return 'Popular';
  if (tool.cat === 'img') return 'Image Tools';
  if (['protect-pdf','unlock-pdf','repair-pdf'].includes(id)) return 'Security & Recovery';
  if (['merge-pdf','split-pdf','compress-pdf','rotate-pdf','delete-pdf-pages','organize-pdf','crop-pdf','page-number','flatten-pdf','pdf-zip-extract'].includes(id)) return 'Organize PDF';
    ...
    <AnimatePresence>{open && <motion.div className="fixed inset-0 z-[250] bg-slate-950/25 px-0 pt-0 backdrop-blur-[3px] sm:px-4 sm:pt-[76px]" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}>
      <motion.div ref={panelRef} role="dialog" aria-modal="true" aria-label={t('common.openAllTools')} initial={reduceMotion?{opacity:0}:{opacity:0,y:-10,scale:.99}} animate={reduceMotion?{opacity:1}:{opacity:1,y:0,scale:1}} exit={reduceMotion?{opacity:0}:{opacity:0,y:-8,scale:.99}} className="mx-auto flex h-[100dvh] w-full max-w-6xl flex-col overflow-hidden bg-white shadow-[0_30px_100px_rgba(15,23,42,.24)] sm:h-[min(78vh,720px)] sm:rounded-[1.4rem] sm:border sm:border-slate-200">
        <div className="border-b border-slate-200 bg-white px-4 py-4 sm:px-6">
          ...
          <div className="relative mt-4"><Search className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-violet-600" /><input autoFocus aria-label="Search all AJN PDF tools" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search: merge, compress, sign, protect, crop, image…" className="h-11 w-full rounded-xl border border-slate-200 bg-slate-50/70 pl-10 pr-4 text-sm font-semibold text-slate-950 outline-none transition focus:border-violet-300 focus:bg-white focus:ring-4 focus:ring-violet-100" /></div>
        </div>

        <div className="flex-1 overflow-y-auto px-4 py-5 sm:px-6 sm:py-6">
          {filtered.length===0?<div className="mx-auto max-w-md py-16 text-center"><Search className="mx-auto h-8 w-8 text-slate-300"/><p className="mt-4 text-base font-black text-slate-800">No matching tool</p><p className="mt-2 text-sm font-medium text-slate-500">Try merge, compress, sign, protect, crop or image.</p></div>:<div className="space-y-8">{groups.map((group)=><section key={group.name}><div className="mb-3 flex items-center justify-between gap-4"><h3 className="text-[11px] font-black uppercase tracking-[.14em] text-slate-500">{group.name}</h3><span className="text-[10px] font-bold text-slate-400">{group.items.length}</span></div><div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">{group.items.map(({item,localized})=><Link key={item.id} href={toolPath(item.id)} prefetch={false} onPointerEnter={()=>router.prefetch(toolPath(item.id))} onFocus={()=>router.prefetch(toolPath(item.id))} onClick={openTool} className="group flex min-h-[68px] items-center gap-3 rounded-xl border border-slate-200/80 bg-white p-2.5 transition hover:-translate-y-px hover:border-violet-200 hover:bg-violet-50/40 hover:shadow-md focus-visible:outline focus-visible:outline-2 focus-visible:outline-violet-500"><ToolArtwork toolId={item.id} toolName={localized.name} className="h-10 w-10 shrink-0"/><div className="min-w-0 flex-1"><p className="truncate text-[12px] font-black text-slate-900 group-hover:text-violet-800">{localized.name}</p><p className="mt-0.5 line-clamp-1 text-[10.5px] font-medium text-slate-500">{localized.desc}</p></div><ArrowRight className="h-3.5 w-3.5 shrink-0 text-slate-300 transition group-hover:translate-x-0.5 group-hover:text-violet-600"/></Link>)}</div></section>)}</div>}
        </div>

        <div className="flex items-center justify-between gap-3 border-t border-slate-200 bg-slate-50/80 px-4 py-3 sm:px-6"><span className="inline-flex items-center gap-1.5 text-[10px] font-bold text-slate-500"><Sparkles className="h-3.5 w-3.5 text-violet-600"/> {BUILD_PUBLIC_TOOLS.length} maintained public workflows</span><Link href="/pdf-tools" onClick={() => setOpen(false)} className="text-[11px] font-black text-violet-700 hover:text-violet-900">Browse directory →</Link></div>
      </motion.div>
