      ...
      <div className="relative mx-auto grid w-full max-w-7xl items-center gap-10 rounded-[2rem] border border-white/70 bg-white/45 p-5 shadow-[0_30px_90px_rgba(26,61,132,.08)] backdrop-blur-sm dark:border-white/5 dark:bg-slate-950/25 md:p-8 lg:grid-cols-[1.02fr_.98fr] lg:gap-12 xl:gap-16 xl:p-10">
        <div className="min-w-0">
          ...
          <motion.h1 {...enter(0.05)} className="mt-6 max-w-3xl text-[clamp(2.75rem,6.4vw,5.6rem)] font-black leading-[.94] tracking-[-.055em] text-slate-950 dark:text-slate-50">
            {t('home.title1')}<span className="mt-1 block"><span className="text-red-600">{t('home.title2').split(' ')[0]}</span> <span className="text-blue-600">{t('home.title2').split(' ').slice(1).join(' ')}</span></span>
          </motion.h1>
          ...
          <motion.div {...enter(0.2)} className="mt-6 flex flex-col gap-3 sm:flex-row"><Link href="/pdf-tools" data-analytics-id="hero-explore-tools"><Button className="h-12 w-full rounded-xl bg-red-600 px-6 text-[12px] font-black text-white shadow-[0_14px_30px_rgba(239,35,60,.22)] hover:bg-red-700 sm:w-auto">{t('home.explore')} <ArrowRight className="ml-2 h-4 w-4" /></Button></Link><Link href="/transparency" data-analytics-id="hero-processing-transparency"><Button variant="outline" className="h-12 w-full rounded-xl border-slate-200 bg-white/90 px-6 text-[12px] font-black text-slate-800 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900/90 dark:text-slate-100 dark:hover:bg-slate-800 sm:w-auto">{t('home.processing')}</Button></Link></motion.div>
        </div>
