...
export function ToolLayout({ children, title, description, category = 'PDF tool', icon = '📄', processingMode = 'browser' }: ToolLayoutProps) {
  ...
  return (
    <div className="ajn-page-shell min-h-screen text-slate-950 dark:text-zinc-50">
      <header className="ajn-tool-header sticky top-0 z-[100] border-b backdrop-blur-2xl">
        <div className="mx-auto flex h-[68px] max-w-7xl items-center justify-between gap-2 px-4 md:h-[72px] md:px-8">
          ...
          <div className="flex items-center gap-2">
            ...
            <ThemeToggle className="ajn-theme-toggle" />
          </div>
        </div>
      </header>
      <main className="relative z-10 mx-auto max-w-5xl px-4 pb-28 pt-8 md:px-8 md:py-16">
        <PremiumBackground />
        <Link href="/pdf-tools" className="relative z-10 inline-flex items-center gap-2 text-xs font-black text-slate-500 transition hover:text-indigo-600 dark:text-zinc-400 dark:hover:text-orange-300"><ChevronLeft className="h-4 w-4" />Back to public tools</Link>
        <motion.div initial={reduceMotion ? false : { opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} className="relative z-10 mt-7 flex flex-col gap-5 sm:flex-row sm:items-center">
          <div className="ajn-tool-hero-icon flex h-16 w-16 shrink-0 items-center justify-center rounded-[1.4rem] text-3xl">{icon}</div>
          <div><h1 className="text-4xl font-black tracking-[-.045em] text-slate-950 dark:text-white md:text-6xl">{title}</h1><p className="mt-3 max-w-2xl text-sm font-medium leading-6 text-slate-600 dark:text-zinc-300 md:text-base">{description}</p></div>
        </motion.div>
        <div className="ajn-liquid-card relative z-10 mt-8 rounded-[2rem] p-3.5 md:mt-10 md:p-6">{children}</div>
      </main>
    </div>
  );
}
