...
export function ToolLayout({ children, title, description, category = 'PDF tool', icon = '📄' }: ToolLayoutProps) {
  ...
  return (
    <div className="ajn-page-shell min-h-screen text-slate-950">
      ...
      <main className="relative z-10 mx-auto max-w-5xl px-4 pb-28 pt-8 md:px-8 md:py-16">
        <Link href="/pdf-tools" className="relative z-10 inline-flex items-center gap-2 text-xs font-black text-slate-500 transition hover:text-blue-700"><ChevronLeft className="h-4 w-4" />Back to tools</Link>
        <motion.div initial={reduceMotion ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="relative z-10 mt-7 flex flex-col gap-5 sm:flex-row sm:items-center">
          <div className="ajn-tool-hero-icon flex h-16 w-16 shrink-0 items-center justify-center rounded-xl text-3xl">{icon}</div>
