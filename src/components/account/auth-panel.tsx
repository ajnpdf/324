...
import { Chrome, Code2, Loader2, LockKeyhole, Mail, UsersRound } from 'lucide-react';
import { useAuth } from '@/lib/auth-context';

type BusyAction = 'email' | 'google' | 'facebook' | 'github' | '';
...
export function AuthPanel({ mode }: { mode: 'login' | 'signup' }) {
  ...
  if (!auth.configured) return <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm font-semibold leading-6 text-amber-950">Firebase Authentication is wired but not configured on this deployment. Run the AJN PDF authentication setup before enabling account features.</div>;

  const socialButtons = [
    { id: 'google' as const, label: 'Google', icon: Chrome, action: auth.signInWithGoogle },
    { id: 'facebook' as const, label: 'Facebook', icon: UsersRound, action: auth.signInWithFacebook },
    { id: 'github' as const, label: 'GitHub', icon: Code2, action: auth.signInWithGithub },
  ];
  ...
  return <div className="rounded-[1.6rem] border border-slate-200 bg-white p-5 shadow-[0_24px_65px_rgba(15,23,42,.08)] sm:p-7">
    <div className="grid gap-2 sm:grid-cols-3">
      {socialButtons.map(({ id, label, icon: Icon, action }) => <button key={id} type="button" disabled={busy} onClick={() => complete(id, action)} className="flex min-h-12 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-3 text-xs font-black text-slate-800 transition hover:border-violet-200 hover:bg-violet-50/40 disabled:opacity-50">{busyAction === id ? <Loader2 className="h-4 w-4 animate-spin"/> : <Icon className="h-4 w-4"/>}<span>{label}</span></button>)}
    </div>
    <p className="mt-3 text-center text-[11px] font-semibold leading-5 text-slate-500">Secure sign-in through Firebase. No guest or anonymous accounts.</p>
