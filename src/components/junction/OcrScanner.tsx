...
export default function OcrScanner() {
  ...
  return (
    <ToolWorkspace
      title="Scan to Text"
      description="Turn photos of documents into selectable digital text — 100% local."
      icon="📸"
      accent={T.teal}
      badge="LIVE OCR"
    >
        ...
        {!text && (
          <>
            ...
            {mode === "camera" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                <div style={{ borderRadius: 32, overflow: "hidden", background: "#000", position: "relative", minHeight: 300, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  ...
                  {!cameraActive && (
                    <p style={{ color: "#64748b", fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em" }}>
                      Camera standby
                    </p>
                  )}
                </div>
            ...
            <Info bg="#F0FDFA" col="#0F766E">
              📷 <strong>Privacy Promise:</strong> All frames are processed in-memory via Tesseract.js. No images are sent to any server.
            </Info>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
              <p style={{ fontSize: 13, fontWeight: 800, margin: 0 }}>
                Extracted Text <span style={{ fontWeight: 400, color: "#6B7280" }}>({wc} words)</span>
              </p>
              <div style={{ display: "flex", gap: 8 }}>
                <Btn variant="secondary" onClick={copy} style={{ padding: "5px 12px", fontSize: 12 }}>
                  {copied ? "✅ Copied!" : "📋 Copy"}
                </Btn>
                <Btn variant="secondary" onClick={reset} style={{ padding: "5px 12px", fontSize: 12 }}>↺ New Scan</Btn>
              </div>
            </div>
            <textarea
              readOnly
              value={text}
              rows={14}
              style={{ width: "100%", border: "1.5px solid #E5E7EB", borderRadius: 16, padding: "16px", fontSize: 13, fontFamily: "monospace", outline: "none", resize: "vertical", lineHeight: 1.7, boxSizing: "border-box", background: "rgba(255,255,255,0.6)" }}
            />
