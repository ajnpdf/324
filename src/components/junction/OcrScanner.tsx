...
export default function OcrScanner(){
  const [files,setFiles]=useState<ToolFile[]>([]);const [mode,setMode]=useState<"upload"|"camera">("upload");const [language,setLanguage]=useState("eng");
  ...
  return <ToolWorkspace title="OCR Scanner" description="Capture or upload a document photo and recognize text in six AJN PDF OCR languages." accent={T.teal}>
    <div className="space-y-5">
      {!text&&<>
        <G2><F label="Input method"><select style={IS} value={mode} onChange={e=>{setMode(e.target.value as any);if(e.target.value==="upload")stopCamera();}}><option value="upload">Upload image</option><option value="camera">Use camera</option></select></F><F label="OCR language"><select style={IS} value={language} onChange={e=>setLanguage(e.target.value)}>{LANGUAGES.map(item=><option key={item.value} value={item.value}>{item.label}</option>)}</select></F></G2>
        ...
        <Info bg="#F0FDFA" col="#0F766E">OCR images are processed by the AJN PDF backend for multilingual recognition and automatic scan cleanup. Temporary processing files are deleted after the job.</Info>
      ...
      {text&&<div className="space-y-4"><Info bg="#ECFDF5" col="#065F46"><strong>{words} words recognized.</strong> Review OCR text before using it in critical records.</Info><textarea readOnly value={text} rows={16} className="w-full resize-y rounded-2xl border border-slate-200 bg-white p-5 font-mono text-sm leading-7 outline-none"/><div className="flex flex-wrap justify-center gap-2"><Btn onClick={copy} style={{background:T.teal}}>{copied?"Copied":"Copy all"}</Btn><Btn variant="secondary" onClick={()=>{const blob=new Blob([text],{type:"text/plain;charset=utf-8"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="ajn-ocr-scan.txt";a.click();URL.revokeObjectURL(url);}}>Download TXT</Btn><Btn variant="secondary" onClick={reset}>Scan another</Btn></div><Err msg={error}/></div>}
    </div>
  </ToolWorkspace>;
}
