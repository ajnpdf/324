...
export default function CropImage(){
  ...
  const run=async()=>{if(!files.length){setE("Upload an image.");return;}setE("");setL(true);
    beginToolProcessing("Crop image");try{setR(await cropImage(files[0].file,x,y,w,h));completeToolProcessing();}catch(e:any){failToolProcessing();setE(e.message || "The task could not be completed.");}setL(false);};
  return(<ToolWorkspace title="Crop Image" description="Cut out a specific rectangular region of your image." accent="#D97706">
