...
export default function HeicToPdf() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (valid.length > 0) {
      setF(valid);
      setPhase('configure');
      setOutputName(valid[0].name.replace(/\.[^/.]+$/, "") + "_iOS_Export");
      ...
      try {
        ...
        console.warn("[AJN] HEIC preview synthesis failed.");
      }
    }
  };
  ...
  const executeConversion = async () => {
    ...
    try {
      ...
      setPhase('configure');
      toast({ title: "Transcode Error", description: "Binary synthesis interrupted.", variant: "destructive" });
    }
  };
  ...
  return (
    <ToolWorkspace title="HEIC to PDF" description="TURN IPHONE PHOTOS INTO STANDARD PDF DOCUMENTS" icon="📱" badge="MEDIA UNIT" accent="#06B6D4">
            ...
            <motion.div key="upload" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full">
              ...
              >
                ...
                <div className="text-center space-y-1 px-8 relative z-10">
                  ...
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">Native iOS Ingestion</p>
                </div>
              </div>
            </motion.div>
          )}
            ...
            <motion.div key="configure" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-8">
              <div className="p-6 bg-white/40 rounded-[2.5rem] border border-black/5 flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-xs font-black text-slate-900 uppercase">{files.length} iOS Assets Loaded</p>
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">WASM In-Memory Ready</p>
                  </div>
                </div>
                <button onClick={reset} className="text-[10px] font-black uppercase text-red-500 hover:underline">Clear Queue</button>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                  ...
                  <Card className="bg-white border-black/5 rounded-[2.5rem] shadow-inner overflow-hidden min-h-[600px] flex items-center justify-center p-12">
                    {previews[0] ? (
                      <div className="relative group shadow-2xl">
                        <img src={previews[0]} className="max-h-[500px] w-auto rounded-sm border border-black/5" alt="" />
                      </div>
                    ) : (
                      <div className="text-center space-y-4 opacity-40">
                        <FileWarning className="w-16 h-16 mx-auto text-slate-300" />
                        <p className="text-[10px] font-black uppercase tracking-widest">Preview Restricted • System Ready</p>
                      </div>
                    )}
                  </Card>
                </div>
                ...
                <aside className="lg:col-span-5 space-y-6">
                    ...
                    <Card className="bg-white/60 backdrop-blur-xl border-black/5 rounded-3xl p-8 space-y-6 shadow-xl border-2">
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          ...
                          <Input placeholder="iOS_Export" value={outputName} onChange={(e) => setOutputName(e.target.value)} className="h-12 pl-12 bg-white/5 border-black/5 rounded-xl font-bold" />
                        </div>
                      </div>
                      <div className="p-4 bg-cyan-500/5 border border-cyan-500/10 rounded-2xl">
                        <p className="text-[9px] font-bold text-cyan-700 uppercase leading-relaxed text-center">
                          HEIC transcodes are performed locally. No data leaves your node.
                        </p>
                      </div>
                    </Card>
                  </section>
                  ...
                  <Button onClick={executeConversion} className="w-full h-16 bg-primary text-white font-black text-xs uppercase tracking-widest rounded-2xl shadow-xl hover:scale-105 transition-all gap-3 border-2 border-white/20 active:scale-95">
                    <Zap className="w-4 h-4" /> Start Transcoding
                  </Button>
                </aside>
              </div>
            </motion.div>
          ...
          {phase === 'done' && resultBlob && (
            <motion.div key="done" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="py-12 flex flex-col items-center space-y-10 text-center pb-32">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm font-bold text-slate-500 uppercase tracking-widest">iOS assets correctly synthesized</p>
              </div>
