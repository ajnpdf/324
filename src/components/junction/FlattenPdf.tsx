...
export default function FlattenPdf() {
  ...
  const executeFlatten = async () => {
    ...
    try {
      ...
      setProgress(80);
      setStatus("Synchronizing binary trailer...");
  ...
  return (
    <ToolWorkspace title="Flatten PDF" description="MERGE INTERACTIVE LAYERS INTO STATIC CONTENT" icon="📄" badge="UTILITY UNIT" accent="#6B7280">
            ...
            <motion.div key="upload" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full">
              ...
              >
                ...
                <div className="text-center space-y-1 px-8 relative z-10">
                  ...
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">Surgical Local Synthesis</p>
                </div>
              </div>
            </motion.div>
          )}
                ...
                <aside className="lg:col-span-5 space-y-6">
                  <section className="space-y-4">
                    <div className="flex items-center gap-2 px-1">
                      ...
                      <Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Operation Setup</Label>
                    </div>
                    ...
                    <Card className="bg-white/60 backdrop-blur-xl border-black/5 rounded-3xl p-8 space-y-6 shadow-xl">
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          ...
                          <Input placeholder="Static_Document" value={outputName} onChange={(e) => setOutputName(e.target.value)} className="h-12 pl-12 bg-white/5 border-black/5 rounded-xl font-bold shadow-sm" />
                        </div>
                      </div>
                  ...
                  <div className="p-6 bg-emerald-500/5 border border-emerald-500/10 rounded-[2rem] flex items-start gap-4">
                    ...
                    <p className="text-[9px] font-bold text-slate-500 uppercase leading-relaxed tracking-wide text-center w-full">
                      Everything runs in your browser memory. Your documents never touch our nodes.
                    </p>
                  </div>
          ...
          {phase === 'done' && resultBlob && (
            <motion.div key="done" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="py-12 flex flex-col items-center space-y-10 text-center">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm font-bold text-slate-500 uppercase tracking-widest">Document correctly flattened</p>
              </div>
