...
/**
 * AJN Professional PDF to EPUB - Production v7.0
 * Specialized in Reflow Logic and eBook Packaging.
 */
export default function PdfToEbook() {
  ...
  const processFile = async (f: File) => {
    ...
    setOutputName(f.name.replace('.pdf', '') + "_Reader");
  ...
  const executeConversion = async () => {
    ...
    setPhase('processing');
    setProgress(0);
    setStatus("Unpacking document flow...");

    try {
      const zip = new JSZip();
      
      // Basic EPUB 2.0 structure
      ...
      zip.file("OEBPS/content.opf", `<?xml version="1.0" encoding="UTF-8"?><package xmlns="http://www.idpf.org/2007/opf" unique-identifier="pub-id" version="2.0"><metadata xmlns:dc="http://purl.org/dc/elements/1.1/"><dc:title>${outputName}</dc:title><dc:language>en</dc:language></metadata><manifest><item href="text.xhtml" id="text" media-type="application/xhtml+xml"/></manifest><spine><itemref idref="text"/></spine></package>`);
      zip.file("OEBPS/text.xhtml", `<?xml version="1.0" encoding="UTF-8"?><!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.1//EN" "http://www.w3.org/TR/xhtml11/DTD/xhtml11.dtd"><html xmlns="http://www.w3.org/1999/xhtml"><head><title>${outputName}</title></head><body>${fullText}</body></html>`);
  ...
  return (
    <ToolWorkspace title="PDF to eBook" description="OPTIMIZE DOCUMENTS FOR KINDLE & E-READERS" icon="📱" badge="MOBILE UNIT" accent="#0D9488">
            ...
            <motion.div key="upload" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full">
              <div 
                ...
                className={cn(
                  ...
                  isDragging ? "border-teal-500 bg-teal-500/10" : "border-black/5 bg-white/20 backdrop-blur-md hover:border-teal-500/40"
                )}
              >
                ...
                <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center shadow-lg mb-6 group-hover:scale-110 transition-transform duration-500 border border-black/5">
                  <TabletSmartphone className="w-8 h-8 text-teal-500" />
                </div>
                <div className="text-center space-y-1 px-8 relative z-10">
                  <h3 className="text-2xl font-black tracking-tighter uppercase text-slate-950">Drop PDF to Convert</h3>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">Reflowable EPUB Synthesis</p>
                </div>
              </div>
            </motion.div>
          )}
              ...
              <div className="p-6 bg-white/40 rounded-[2.5rem] border border-black/5 flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{fmtBytes(file.size)} • {pageCount} Segments Detected</p>
                  </div>
                </div>
                ...
                <aside className="lg:col-span-5 space-y-6">
                  <section className="space-y-4">
                    <div className="flex items-center gap-2 px-1">
                      ...
                      <Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">eBook Setup</Label>
                    </div>
                    ...
                    <Card className="bg-white/60 backdrop-blur-xl border-black/5 rounded-3xl p-8 space-y-6 shadow-xl">
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          ...
                          <Input placeholder="Reader_Export" value={outputName} onChange={(e) => setOutputName(e.target.value)} className="h-12 pl-12 bg-white/5 border-black/5 rounded-xl font-bold shadow-sm" />
                        </div>
                      </div>
                      <div className="p-4 bg-teal-500/5 border border-teal-500/10 rounded-2xl">
                        <p className="text-[9px] font-bold text-teal-700 uppercase leading-relaxed text-center">
                          PDF segments will be converted to XHTML flowing text for standard EPUB compatibility.
                        </p>
                      </div>
                    </Card>
                  </section>
          ...
          {phase === 'done' && resultBlob && (
            <motion.div key="done" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="py-12 flex flex-col items-center space-y-10 text-center">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm font-bold text-slate-500 uppercase tracking-widest">eBook correctly synthesized</p>
              </div>
              ...
              <div className="p-8 bg-white border-2 border-black/5 rounded-[3rem] w-full max-w-sm flex items-center justify-center gap-4 shadow-xl mx-auto">
                ...
                <div className="text-left overflow-hidden">
                  ...
                  <p className="text-sm font-black text-slate-950 truncate">{outputName}.epub</p>
                </div>
              </div>

              <div className="w-full max-w-sm flex flex-col gap-4 mx-auto pt-4 pb-32">
                <Button onClick={() => dl(resultBlob, `${outputName}.epub`)} className="h-16 bg-emerald-500 text-white font-black text-sm uppercase tracking-widest rounded-2xl shadow-xl hover:bg-emerald-600 transition-all gap-3 border-2 border-white/20 active:scale-95">
