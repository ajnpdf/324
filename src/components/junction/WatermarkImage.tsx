...
export default function WatermarkImage(){
  ...
  return(<ToolWorkspace title="Watermark Image" description="Add your brand or copyright text to any image." icon="💧" accent="#06B6D4">
    ...
    :<div style={{display:"flex",flexDirection:"column",gap:16}}>
      ...
      <G2><Range label="Opacity" value={Math.round(opacity*100)} min={10} max={100} step={5} onChange={v=>setOp(v/100)} fmt={v=>`${v}%`}/>
          <Range label="Font size (px)" value={size} min={12} max={120} step={4} onChange={setSize}/></G2>
      <G2>
        <F label="Text color"><input type="color" value={color} onChange={e=>setColor(e.target.value)} style={{width:"100%",height:40,borderRadius:8,border:`1.5px solid ${T.border}`,cursor:"pointer",padding:2}}/></F>
        <F label="Position"><select style={SS} value={pos} onChange={e=>setPos(e.target.value)}>
