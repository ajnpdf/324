...
import { ToolWorkspace, Drop, Btn, Done, Err, F, IS, Pills, ToolFile, dl, Info } from "./_shared";
...
export default function SplitPdf() {
  ...
  const process = async () => {
    ...
    try {
      ...
      setResult({ blob, name: parts.length === 1 ? parts[0].name : `${prefix.trim() || "split"}_files.zip`, count: parts.length });
