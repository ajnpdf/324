...
import { ToolWorkspace, Drop, Range, ToolFile, dl, T, fmtBytes, shareResult, beginToolProcessing, completeToolProcessing, failToolProcessing} from "./_shared";
...
import { 
  ...
  Zap,
  Download,
  Loader2, Share2} from "lucide-react";
...
export default function PhotoEditor() {
  ...
  const run = async () => {
    ...
    setE(""); setL(true);
    beginToolProcessing("PhotoEditor");
    setPhase('processing');
    try {
          ...
          {phase === 'done' && result && (
            <motion.div key="done" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="py-12 flex flex-col items-center space-y-10 text-center">
              ...
              <div className="w-full max-w-sm flex flex-col gap-4 mx-auto pt-4 pb-32">
                ...
                <Button variant="outline" onClick={() => { if (result) void shareResult(result, "Edited_Photo.jpg"); }} className="h-12 border-slate-200 bg-white text-slate-700 font-black text-xs rounded-xl shadow-sm hover:border-blue-200 hover:bg-blue-50/60 gap-2">
