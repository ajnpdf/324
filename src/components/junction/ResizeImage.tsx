...
export default function ResizeImage() {
  ...
  return (
    <ToolWorkspace title="Resize Image" description="PIXEL-PERFECT DIMENSION SCALING" icon="📐" accent="#2563EB" badge="SCALING UNIT">
              ...
              <div className="p-6 bg-white/40 rounded-[2.5rem] border border-black/5 flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{fmtBytes(files[0].size)} &bull; Master Buffer Active</p>
                  </div>
                </div>
                <button onClick={reset} className="text-[10px] font-black uppercase text-red-500 hover:underline">Clear Workspace</button>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div className="lg:col-span-7 space-y-3">
                  <Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Scaling Viewport</Label>
                  <Card className="bg-slate-900/5 border-black/5 rounded-[2.5rem] shadow-inner overflow-hidden min-h-[500px] flex items-center justify-center p-12">
                    <div className="relative group shadow-2xl">
                      <img src={preview} className="max-h-[400px] w-auto rounded-sm border border-black/5 transition-all" alt="" />
                    </div>
                  </Card>
                </div>
                ...
                <aside className="lg:col-span-5 space-y-6">
                  <section className="space-y-4">
                    ...
                    <Card className="bg-white/60 backdrop-blur-xl border-black/5 rounded-3xl p-8 space-y-8 shadow-xl border-2">
                      ...
                      <div className="space-y-3">
                        <Label className="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-1">Universal Presets</Label>
                  ...
                  <Button onClick={run} className="w-full h-16 bg-primary text-white font-black text-xs uppercase tracking-widest rounded-2xl shadow-xl hover:scale-105 transition-all gap-3 border-2 border-white/20 active:scale-95">
                    <Zap className="w-4 h-4" /> Apply Scaling
                  </Button>
                </aside>
              </div>
            </motion.div>
          ...
          {phase === 'done' && resultBlob && (
            <motion.div key="done" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="py-12 flex flex-col items-center space-y-10 text-center pb-32">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm font-bold text-slate-500 uppercase tracking-widest">Image correctly scaled to {w} &times; {h} px</p>
              </div>
