...
export default function ResizeImage() {
  ...
  useEffect(()=>{
    ...
    const url=URL.createObjectURL(files[0].file); const img=new Image();
    img.onload=()=>{setWidth(img.naturalWidth);setHeight(img.naturalHeight);setOutputName(`${files[0].name.replace(/\.[^/.]+$/,"")}-resized.png`);}; img.src=url; setPreview(url);
  ...
  const name=safeOutputName(outputName,"resized",".png");
  return <ToolWorkspace title="Resize Image" description="Resize an image to the dimensions you need" accent="#2563EB">
    {result?<Done msg="Image resized" onDownload={()=>dl(result,name)} shareFile={{blob:result,name}} onReset={()=>{setResult(null);setFiles([]);setError("");}}/>:<div className="space-y-4">
      ...
      <F label="Output filename"><input style={IS} value={outputName} onChange={e=>setOutputName(e.target.value)}/></F><Err msg={error}/><Btn onClick={run} loading={loading} disabled={!files.length} full>Resize image</Btn>
    </div>}
  </ToolWorkspace>;
}
