...
export default function SubtitleGenerator() {
  ...
  const executeExport = async () => {
    setPhase('processing');
    setProgress(0);
    setStatus("Synthesizing binary headers...");
  ...
  return (
    <ToolWorkspace title="Subtitle Creator" description="GENERATE TIMED SRT OR VTT CAPTIONS LOCALLY" icon="💬" badge="VIDEO UNIT" accent="#4F46E5">
            ...
            <motion.div key="upload" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full max-w-4xl mx-auto space-y-10">
              ...
              >
                ...
                <div className="text-center space-y-1 px-8">
                  ...
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">Local Timing Synthesis</p>
                </div>
              </div>

              <div className="bg-white/40 border-2 border-black/5 rounded-[3rem] p-10 space-y-6 shadow-2xl backdrop-blur-xl">
                <Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Manual Content Ingestion</Label>
                <Textarea 
                ...
                <Button onClick={autoSplit} disabled={!inputText.trim()} className="w-full h-14 bg-primary text-white font-black text-xs uppercase tracking-widest rounded-2xl shadow-xl hover:scale-[1.02] transition-all gap-3 border-2 border-white/20">
                  <Zap className="w-4 h-4" /> Initialize Auto-Split
                </Button>
              </div>
            </motion.div>
          )}
            ...
            <motion.div key="configure" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-8">
              <div className="p-6 bg-white/40 rounded-[2.5rem] border border-black/5 flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-xs font-black text-slate-900 uppercase">Interactive Workspace</p>
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{segments.length} Segments Mapped</p>
                  </div>
                </div>
                <button onClick={reset} className="text-[10px] font-black uppercase text-red-500 hover:underline">Flush Session</button>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div className="lg:col-span-8 space-y-3">
                  <Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Timeline Calibration</Label>
                ...
                <aside className="lg:col-span-4 space-y-6">
                  <section className="space-y-4">
                    <div className="flex items-center gap-2 px-1">
                      ...
                      <Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Logic Control</Label>
                    </div>
                    ...
                    <Card className="bg-white/60 backdrop-blur-xl border-black/5 rounded-3xl p-8 space-y-8 shadow-xl">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          ...
                          <Input placeholder="Movie_Captions" value={outputName} onChange={(e) => setOutputName(e.target.value)} className="h-12 pl-12 bg-white/5 border-black/5 rounded-xl font-bold" />
                        </div>
                      </div>
                      ...
                      <div className="pt-6 border-t border-black/5 space-y-6">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-[10px] font-black uppercase tracking-widest">Auto-Recalibrate</span>
                        </div>
                        <Button variant="outline" onClick={autoSplit} className="w-full h-10 border-black/5 bg-white font-black text-[9px] uppercase tracking-widest hover:bg-primary hover:text-white rounded-xl shadow-sm transition-all">Re-Apply Split Rules</Button>
                      </div>
                    </Card>
                  </section>
                  ...
                  <Button onClick={executeExport} className="w-full h-16 bg-primary text-white font-black text-xs uppercase tracking-widest rounded-2xl shadow-xl hover:scale-105 transition-all gap-3 border-2 border-white/20 active:scale-95">
                    <Zap className="w-4 h-4" /> Generate Binary
                  </Button>
                </aside>
              </div>
            </motion.div>
          ...
          {phase === 'done' && (
            <motion.div key="done" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="py-12 flex flex-col items-center space-y-10 text-center">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm font-bold text-slate-500 uppercase tracking-widest">Subtitle binary correctly generated</p>
              </div>
