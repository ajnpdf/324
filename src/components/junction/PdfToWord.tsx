...
/**
 * AJN Professional PDF to Word - Production v9.0
 * Specialized in Paragraph Reconstruction and OOXML Synthesis.
 */
export default function PdfToWord() {
  ...
  const processFile = async (f: File) => {
    setFile(f);
    setPhase('configure');
    setStatus("Analyzing structural layout...");
    setOutputName(f.name.replace('.pdf', '') + "_Editable");
  ...
  const executeConversion = async () => {
    ...
    setPhase('processing');
    setProgress(0);
    setStatus("Reconstructing paragraphs...");
    ...
    try {
      ...
      setResultBlob(new Blob([await blob.arrayBuffer()], { type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' }));
      setPhase('done');
    } catch (err) {
      setPhase('configure');
  ...
  return (
    <ToolWorkspace title="PDF to Word" description="SURGICAL PARAGRAPH RECONSTRUCTION" icon="📝" badge="OFFICE UNIT" accent="#2563EB">
            ...
            <motion.div key="upload" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full">
              ...
              >
                ...
                <div className="text-center space-y-1 px-8 relative z-10">
                  ...
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">Local OOXML Transcoding</p>
                </div>
              </div>
            </motion.div>
          )}
              ...
              <div className="p-6 bg-white/40 rounded-[2.5rem] border border-black/5 flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{fmtBytes(file.size)} • {pageCount} Pages • Safe Buffer</p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <aside className="lg:col-span-5 space-y-6">
                    ...
                    <Card className="bg-white/60 backdrop-blur-xl border-black/5 rounded-3xl p-8 space-y-6 shadow-xl">
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          ...
                          <Input placeholder="Editable_Document" value={outputName} onChange={(e) => setOutputName(e.target.value)} className="h-12 pl-12 bg-white/5 border-black/5 rounded-xl font-bold shadow-sm" />
                        </div>
                      </div>
                      <div className="p-4 bg-primary/5 rounded-2xl border border-primary/10">
                        <p className="text-[9px] font-bold text-slate-500 uppercase leading-relaxed text-center">
                          Neural reconstruction attempts to group characters into logical paragraphs for Microsoft Word compatibility.
                        </p>
                      </div>
                    </Card>
                  </section>
                  ...
                  <Button onClick={executeConversion} className="w-full h-16 bg-primary text-white font-black text-xs uppercase tracking-widest rounded-2xl shadow-xl hover:scale-105 transition-all gap-3 border-2 border-white/20 active:scale-95">
                    <Zap className="w-4 h-4" /> Start Synthesis
                  </Button>
                </aside>
              </div>
            </motion.div>
          ...
          {phase === 'done' && resultBlob && (
            <motion.div key="done" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="py-12 flex flex-col items-center space-y-10 text-center">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm font-bold text-slate-500 uppercase tracking-widest">Document correctly synthesized</p>
              </div>
              ...
              <div className="p-8 bg-white border-2 border-black/5 rounded-[3rem] w-full max-w-sm flex items-center justify-center gap-4 shadow-xl mx-auto">
                ...
                <div className="text-left overflow-hidden">
                  ...
                  <p className="text-sm font-black text-slate-950 truncate">{outputName}.docx</p>
                </div>
              </div>

              <div className="w-full max-w-sm flex flex-col gap-4 mx-auto pt-4 pb-32">
                <Button onClick={() => dl(resultBlob, `${outputName}.docx`)} className="h-16 bg-emerald-500 text-white font-black text-sm uppercase tracking-widest rounded-2xl shadow-xl hover:bg-emerald-600 transition-all gap-3 border-2 border-white/20 active:scale-95">
