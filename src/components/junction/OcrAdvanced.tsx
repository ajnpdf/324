...
export default function OcrAdvanced() {
  ...
  const runOcr = async () => {
    ...
    setProgress(5); setStatus("Inhaling binary buffer...");
    ...
    try {
      ...
      setProgress(20); setStatus("Executing computer vision pipeline...");
  ...
  return (
    <ToolWorkspace title="Neural OCR" description="ADVANCED NEURAL EXTRACTION & VISION PIPELINE" icon="🧠" badge="UPGRADED ENGINE" accent={T.purple}>
      <div className="w-full">
        <AnimatePresence mode="wait">
          {phase === 'upload' && (
            <motion.div key="upload" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full">
              <Drop files={files} onChange={setF} accept=".pdf,.png,.jpg,.jpeg,.webp" label="Drop Scan to Process" sub="Local binary ingestion active" />
            </motion.div>
          )}
              ...
              <div className="p-6 bg-white/40 rounded-[2.5rem] border border-black/5 flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Sovereign Buffer Ready</p>
                  </div>
                </div>
                   ...
                   <Card className="bg-white/60 backdrop-blur-xl border-black/5 rounded-[2.5rem] p-10 space-y-10 shadow-xl border-2">
                      <div className="grid grid-cols-2 gap-10">
                        <div className="space-y-4">
                          <Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Vision Logic</Label>
                          <div className="space-y-3">
                            <label className="flex items-center justify-between p-3 bg-black/5 rounded-xl cursor-pointer">
                              <span className="text-xs font-black uppercase">Auto-Deskew</span>
                              <input type="checkbox" checked={config.deskew} onChange={e => setConfig({...config, deskew: e.target.checked})} />
                            </label>
                            <label className="flex items-center justify-between p-3 bg-black/5 rounded-xl cursor-pointer">
                              <span className="text-xs font-black uppercase">Denoising</span>
                              <input type="checkbox" checked={config.denoise} onChange={e => setConfig({...config, denoise: e.target.checked})} />
                            </label>
                            <label className="flex items-center justify-between p-3 bg-black/5 rounded-xl cursor-pointer">
                              <span className="text-xs font-black uppercase">Otsu Threshold</span>
                              <input type="checkbox" checked={config.autoThreshold} onChange={e => setConfig({...config, autoThreshold: e.target.checked})} />
                            </label>
                          </div>
                        </div>
                        <div className="space-y-6">
                           <Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Neural Parameters</Label>
                           <div className="p-6 bg-purple-500/5 border border-purple-500/10 rounded-2xl">
                             <p className="text-[11px] font-bold text-purple-700 leading-relaxed uppercase">The engine will utilize multi-stage binarization to maximize character edge recognition on low-quality scans.</p>
                           </div>
                        </div>
                      </div>
                   </Card>
                   ...
                   <div className="p-8 bg-slate-900 text-white rounded-[3rem] shadow-2xl relative overflow-hidden group h-full flex flex-col justify-center">
                    ...
                    <div className="relative z-10 space-y-8">
                      <div className="space-y-2">
                        <h4 className="text-2xl font-black uppercase italic tracking-tighter">Initialize Scan</h4>
            ...
            <motion.div key="done" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="space-y-10 pb-32">
              <div className="p-8 bg-white border-2 border-black/5 rounded-[3rem] shadow-xl flex flex-col md:flex-row items-center justify-between gap-8">
                 <div className="flex items-center gap-6">
                    ...
                    <div>
                       <h3 className="text-2xl font-black uppercase tracking-tighter">Extraction Success</h3>
                       <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{stats?.wordCount} Words correctly mapped</p>
                    </div>
                 </div>
                 <div className="flex items-center gap-4">
                    ...
                    <Button variant="outline" onClick={reset} className="h-12 px-8 rounded-xl font-black text-[10px] uppercase gap-2 border-black/5 bg-white hover:bg-black/5 shadow-sm">
                       <RefreshCcw className="w-4 h-4" /> New Session
                    </Button>
                 </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                 ...
                 <aside className="lg:col-span-4 space-y-6">
                    <Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Statistical Metadata</Label>
                    <Card className="bg-white/60 border-black/5 rounded-[2.5rem] p-8 space-y-8 shadow-xl border-2">
                       {[
                         ...
                         { label: "Character Index", value: resultText.length, status: "Binary mapped" },
                    ...
                    <div className="p-6 bg-emerald-500/5 border border-emerald-500/10 rounded-[2rem] flex items-center gap-3">
                       ...
                       <span className="text-[9px] font-black uppercase tracking-widest text-emerald-600">Privacy directive verified</span>
                    </div>
                 </aside>
              </div>
            </motion.div>
