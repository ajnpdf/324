...
import { ToolWorkspace, fmtBytes } from './_shared';
...
export default function ComparePdf() {
  ...
  const executeComparison = async () => {
    ...
    setPhase('processing');
    setProgress(0);
    setStatus("Inhaling binary versions...");
    ...
    try {
      ...
      console.error(err);
      toast({ title: "Audit failed", description: "Binary comparison interrupted.", variant: "destructive" });
      setPhase('upload');
    }
  };

  return (
    <ToolWorkspace title="Compare PDF" description="See the differences between two versions of a PDF." icon="⚖️" badge="ANALYSIS UNIT" accent="#4F46E5">
            ...
            <motion.div key="upload" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full space-y-10">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div 
                  ...
                  onDrop={e => { e.preventDefault(); setIsDragging({ ...isDragging, a: false }); handleFileUpload(e as any, 'a'); }}
                  className={cn(
                ...
                <div 
                  ...
                  onDrop={e => { e.preventDefault(); setIsDragging({ ...isDragging, b: false }); handleFileUpload(e as any, 'b'); }}
                  className={cn(
                    "group relative h-[280px] rounded-[3rem] border-4 border-dashed transition-all duration-500 shadow-xl overflow-hidden flex flex-col items-center justify-center cursor-pointer",
              ...
              <div className="flex flex-col items-center gap-6">
                ...
                >
                  <Diff className="w-4 h-4" /> Start Comparison
                </Button>
                <div className="flex items-center gap-3 px-6 py-2 bg-white/40 border border-black/5 rounded-full shadow-sm">
                  <ShieldCheck className="w-4 h-4 text-emerald-600" />
                  <span className="text-[9px] font-black uppercase tracking-widest text-slate-500 leading-none">Safe Local session buffer active</span>
                </div>
              </div>
            </motion.div>
          )}
            ...
            <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-10 pb-32">
              <div className="flex flex-col md:flex-row items-center justify-between gap-8 p-8 bg-white border-2 border-black/5 rounded-[3rem] shadow-xl">
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    <h3 className="text-2xl font-black uppercase tracking-tighter text-slate-900">Audit Summary</h3>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Verification complete across {summary.totalPages} pages</p>
                  </div>
                </div>
                ...
                <div className="flex gap-8">
                  ...
                  <div className="flex items-center gap-3 pl-8 border-l border-black/5">
                    <Button onClick={reset} variant="outline" className="h-11 rounded-xl text-[10px] font-black uppercase tracking-widest px-6 gap-2">
                      <RefreshCcw className="w-3.5 h-3.5" /> New Compare
                    </Button>
                  </div>
                </div>
              </div>
                  ...
                  <div key={page.pageIndex} className="space-y-6">
                    <div className="flex items-center gap-4 px-2">
                      ...
                      {page.marks.length === 0 ? (
                        <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest flex items-center gap-2">
                          <CheckCircle2 className="w-3.5 h-3.5" /> No Changes Detected
                        </span>
                      ) : (
                        <span className="text-[10px] font-black text-amber-600 uppercase tracking-widest flex items-center gap-2">
                          <AlertCircle className="w-3.5 h-3.5" /> {page.marks.length} Potential Discrepancies
                        </span>
                      )}
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <Card className="bg-white border-black/5 shadow-2xl rounded-[2.5rem] overflow-hidden relative">
                        ...
                        <div className="p-10 flex justify-center bg-slate-100/30">
                          <div className="relative shadow-2xl border border-black/5 bg-white">
                            {page.previewA ? <img src={page.previewA} className="w-full h-auto" alt="" /> : <div className="aspect-[1/1.4] w-[400px] bg-slate-50 flex items-center justify-center"><FileWarning className="w-8 h-8 text-slate-200" /></div>}
                      ...
                      <Card className="bg-white border-black/5 shadow-2xl rounded-[2.5rem] overflow-hidden relative">
                        ...
                        <div className="p-10 flex justify-center bg-slate-100/30">
                          <div className="relative shadow-2xl border border-black/5 bg-white">
                            {page.previewB ? <img src={page.previewB} className="w-full h-auto" alt="" /> : <div className="aspect-[1/1.4] w-[400px] bg-slate-50 flex items-center justify-center"><FileWarning className="w-8 h-8 text-slate-200" /></div>}
                            {page.marks.filter(m => m.type !== 'removed').map((mark, mi) => (
