...
export default function AddText() {
  ...
  const executeAddText = async () => {
    ...
    setPhase('processing');
    setProgress(0);
    setStatus("Synthesizing text layer...");
  ...
  return (
    <ToolWorkspace title="Add Text" description="WRITE CUSTOM TEXT ANYWHERE ON YOUR PDF PAGES" icon="✏️" badge="EDIT UNIT" accent="#0369A1">
            ...
            <motion.div key="upload" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full">
              ...
              >
                ...
                <div className="text-center space-y-1 px-8 relative z-10">
                  <h3 className="text-2xl font-black tracking-tighter uppercase text-slate-950">Drop PDF to Annotate</h3>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">Direct Binary Stamping</p>
                </div>
              </div>
            </motion.div>
          )}
              ...
              <div className="p-6 bg-white/40 rounded-[2.5rem] border border-black/5 flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{fmtBytes(file.size)} • {pageCount} Pages • Safe Buffer Ready</p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <aside className="lg:col-span-5 space-y-6">
                  <section className="space-y-4">
                    ...
                    <Card className="bg-white/60 backdrop-blur-xl border-black/5 rounded-3xl p-8 space-y-8 shadow-xl">
                      ...
                      <div className="grid grid-cols-2 gap-6">
                        <div className="space-y-2">
                          <Label className="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-1">X Position</Label>
                          <Input type="number" value={settings.x} onChange={(e) => setSettings({...settings, x: parseInt(e.target.value) || 0})} className="h-11 bg-white/5 border-black/5 rounded-xl font-bold" />
                        </div>
                        <div className="space-y-2">
                          <Label className="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-1">Y Position</Label>
                          <Input type="number" value={settings.y} onChange={(e) => setSettings({...settings, y: parseInt(e.target.value) || 0})} className="h-11 bg-white/5 border-black/5 rounded-xl font-bold" />
                        </div>
                      </div>
                  ...
                  <Button onClick={executeAddText} disabled={!settings.text.trim()} className="w-full h-16 bg-primary text-white font-black text-xs uppercase tracking-widest rounded-2xl shadow-xl hover:scale-105 transition-all gap-3 border-2 border-white/20 active:scale-95">
                    <Zap className="w-4 h-4" /> Apply Text Layer
                  </Button>
                </aside>
              </div>
            </motion.div>
          ...
          {phase === 'done' && resultBlob && (
            <motion.div key="done" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="py-12 flex flex-col items-center space-y-10 text-center">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm font-bold text-slate-500 uppercase tracking-widest">Document correctly modified</p>
              </div>
