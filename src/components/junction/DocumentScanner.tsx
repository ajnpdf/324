...
export default function DocumentScanner() {
  ...
  const buildPdf = async () => {
    ...
    try {
      ...
      setE("Assembly Error: Local binary synthesis interrupt.");
    }
    setL(false);
  };
  ...
  return (
    <ToolWorkspace title="Surgical Scanner" description="REAL-TIME EDGE DETECTION & PERSPECTIVE WARPING" icon="📄" accent={T.teal} badge="VISION UNIT">
          ...
          {result ? (
            <motion.div key="done" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
              <div className="text-center p-12 bg-white/40 rounded-[3rem] border border-black/5 shadow-2xl">
                ...
                <h3 className="text-3xl font-black uppercase tracking-tight text-slate-950 mb-2">Scan Synchronized</h3>
                <p className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-12">{captures.length} pages correctly mapped into binary.</p>
            ...
            <motion.div key="scanner" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
              ...
              <div className="flex flex-col items-center gap-6">
                <div className="flex flex-wrap gap-4 justify-center">
                  {!cameraActive ? (
                    <Button onClick={startCamera} disabled={!engineReady} className="h-16 px-12 bg-primary text-white font-black text-xs uppercase tracking-widest rounded-2xl shadow-xl hover:scale-105 transition-all gap-3 border-2 border-white/20 activation:scale-95">
                      {engineReady ? <><Camera className="w-5 h-5" /> Start Viewfinder</> : <><Loader2 className="w-5 h-5 animate-spin" /> Calibrating...</>}
                    </Button>
                  ) : (
                ...
                <div className="flex items-center gap-4 py-2 opacity-60">
                   <div className="flex items-center gap-2"><ShieldCheck className="w-3.5 h-3.5 text-emerald-600" /><span className="text-[8px] font-black uppercase tracking-widest">Local Session</span></div>
                   <div className="w-1 h-1 rounded-full bg-slate-300" />
                   <div className="flex items-center gap-2"><Activity className="w-3.5 h-3.5 text-primary" /><span className="text-[8px] font-black uppercase tracking-widest">Vision Pipeline Active</span></div>
                </div>
              </div>
                    ...
                    <div className="flex flex-wrap gap-4">
                      {captures.map((cap, i) => (
                        <motion.div key={cap.id} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="relative group w-24 h-32 rounded-2xl overflow-hidden border-4 border-white shadow-xl bg-slate-100">
                          <img src={cap.dataUrl} alt="" className="w-full h-full object-cover" />
                    ...
                    <div className="pt-6 flex flex-col gap-4">
                      {loading && (
                        <div className="space-y-2">
                           <div className="flex justify-between text-[10px] font-black uppercase tracking-widest text-primary"><span>Synthesizing Document...</span><span>{compileProgress}%</span></div>
                      ...
                      <Button onClick={buildPdf} disabled={loading} className="w-full h-16 bg-slate-900 text-white font-black text-xs uppercase tracking-[0.2em] rounded-[2rem] shadow-2xl hover:scale-[1.02] transition-all gap-3 border-2 border-white/10">
                        {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <><Download className="w-5 h-5" /> Finalize and Compile PDF</>}
                      </Button>
                    </div>
                  </motion.div>
                )}
