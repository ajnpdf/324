...
/**
 * AJN Professional XML/JSON to PDF - Production v7.0
...
export default function XmlToPdf() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setRawText(text);
    setPhase('configure');
    setOutputName(f.name.replace(/\.[^/.]+$/, "") + "_Data_Report");
  };
  ...
  const executeConversion = async () => {
    ...
    try {
      ...
      const title = `DATA REPORT: ${file?.name || 'Asset'}`;
  ...
  return (
    <ToolWorkspace title="XML/JSON to PDF" description="TURN STRUCTURED DATA INTO READABLE REPORTS" icon="📊" badge="DATA UNIT" accent="#6B7280">
        ...
        <AnimatePresence mode="wait">
          {phase === 'upload' && (
            <motion.div key="upload" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full">
              ...
              >
                <input type="file" accept=".xml,.json,.txt" ref={fileInputRef} className="hidden" onChange={handleFileUpload} />
                ...
                <div className="text-center space-y-1 px-8 relative z-10">
                  <h3 className="text-2xl font-black tracking-tighter uppercase text-slate-950">Drop Data File</h3>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">Local Schema Rendering</p>
                </div>
              </div>
            </motion.div>
          )}
            ...
            <motion.div key="configure" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-8">
              <div className="p-6 bg-white/40 rounded-[2.5rem] border border-black/5 flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{fmtBytes(file.size)} • In-Memory Parsing Active</p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div className="lg:col-span-7 space-y-3">
                  <Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Data Stream Preview</Label>
                ...
                <aside className="lg:col-span-5 space-y-6">
                  <section className="space-y-4">
                    <div className="flex items-center gap-2 px-1">
                      ...
                      <Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Report Setup</Label>
                    </div>
                    ...
                    <Card className="bg-white/60 backdrop-blur-xl border-black/5 rounded-3xl p-8 space-y-6 shadow-xl">
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          ...
                          <Input placeholder="Data_Report" value={outputName} onChange={(e) => setOutputName(e.target.value)} className="h-12 pl-12 bg-white/5 border-black/5 rounded-xl font-bold shadow-sm" />
                        </div>
                      </div>
                      <div className="p-4 bg-slate-500/5 rounded-2xl">
                        <p className="text-[9px] font-bold text-slate-500 uppercase leading-relaxed text-center">
                          Synthesis will map raw structured data into a formal monospaced document for technical auditing.
                        </p>
                      </div>
                    </Card>
                  </section>
          ...
          {phase === 'processing' && (
            <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="py-24 flex flex-col items-center space-y-10 text-center">
              ...
              <div className="w-full max-w-sm space-y-4 mx-auto">
                <div className="flex justify-between items-center px-2"><span className="text-[10px] font-black uppercase tracking-[0.3em] text-primary">Executing Render</span><span className="text-xl font-black text-primary tracking-tighter">{progress}%</span></div>
          ...
          {phase === 'done' && resultBlob && (
            <motion.div key="done" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="py-12 flex flex-col items-center space-y-10 text-center">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm font-bold text-slate-500 uppercase tracking-widest">Report correctly generated</p>
              </div>
