...
import { ToolWorkspace, dl, fmtBytes, Pills, F } from './_shared';

/**
 * AJN Professional TXT to PDF - Production v8.0
 * Specialized in Clean Text-to-Vector Synthesis.
...
export default function TxtToPdf() {
  ...
  return (
    <ToolWorkspace title="TXT to PDF" description="TURN RAW NOTES INTO FORMAL DOCUMENTS" icon="📝" badge="OFFICE UNIT" accent="#2563EB">
              ...
              {inputMode === 'upload' ? (
                ...
                >
                  ...
                  <div className="text-center space-y-1 px-8 relative z-10">
                    ...
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">Local Character Mapping</p>
                  </div>
                </div>
              ) : (
                <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="space-y-6">
                  ...
                  >
                    <Zap className="w-4 h-4" /> Initialize Synthesis
                  </Button>
                </motion.div>
              )}
            </motion.div>
              ...
              <div className="p-6 bg-white/40 rounded-[2.5rem] border border-black/5 flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{file ? fmtBytes(file.size) : `${rawText.length} Characters`} • Buffer Ready</p>
                  </div>
                </div>
                <button onClick={reset} className="text-[10px] font-black uppercase text-red-500 hover:underline">Discard All</button>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div className="lg:col-span-7 space-y-3">
                  <Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Content Preview</Label>
                ...
                <aside className="lg:col-span-5 space-y-6">
                  <section className="space-y-4">
                    <div className="flex items-center gap-2 px-1">
                      ...
                      <Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Document Setup</Label>
                    </div>
                    ...
                    <Card className="bg-white/60 backdrop-blur-xl border-black/5 rounded-3xl p-8 space-y-6 shadow-xl">
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          ...
                          <Input placeholder="Formal_Export" value={outputName} onChange={(e) => setOutputName(e.target.value)} className="h-12 pl-12 bg-white/5 border-black/5 rounded-xl font-bold shadow-sm" />
                        </div>
                      </div>
                      <div className="p-4 bg-primary/5 rounded-2xl">
                        <p className="text-[9px] font-bold text-slate-500 uppercase leading-relaxed text-center">
                          TXT data is synthesized as high-fidelity vector text on A4 standard pages.
                        </p>
                      </div>
                    </Card>
                  </section>
