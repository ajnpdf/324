...
export default function MemeMaker() {
  ...
  const [bottom, setBottom] = useState("BUT CREATE 3 MORE");
  const [fontSize, setFontSize] = useState(0);
  const [outputName, setOutputName] = useState("meme.jpg");
  ...
  useEffect(() => {
    ...
    setPreview(url);
    setOutputName(`${files[0].name.replace(/\.[^/.]+$/, "")}-meme.jpg`);
  ...
  const run = async () => {
    ...
    try { setResult(await withProcessingActivity("Create meme", () => makeMeme(files[0].file, top, bottom, fontSize))); }
    catch (e: any) { setError(e.message || "The meme could not be created."); }
    finally { setLoading(false); }
  };
  const name = safeOutputName(outputName, "meme", ".jpg");

  return <ToolWorkspace title="Meme Maker" description="Add clear top and bottom captions to an image" accent="#F59E0B">
    {result ? <Done msg="Meme created" onDownload={() => dl(result, name)} shareFile={{blob:result,name}} onReset={() => { setResult(null); setFiles([]); setError(""); }}/>
