...
export async function GET() {
  ...
  const entries = posts.map((post) => {
    ...
    return `<url><loc>${SITE_URL}/discover/${xmlEscape(post.slug)}</loc><image:image><image:loc>${xmlEscape(image)}</image:loc><image:title>${xmlEscape(post.title)}</image:title><image:caption>${xmlEscape(post.caption)}</image:caption></image:image></url>`;
  }).join('');
