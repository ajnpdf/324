...
import { ArrowLeftRight, FileImage, FileText, Layers, Search, X } from 'lucide-react';
...
function PDFToolsContent() {
  ...
  return <div className="ajn-page-shell"><Navbar /><main className="relative z-10 pb-24 pt-28 md:pt-36"><section className="mx-auto w-full max-w-7xl px-4 md:px-6 xl:px-8">
    <div className="mx-auto max-w-4xl text-center"><span className="ajn-section-kicker">All AJN PDF tools</span><h1 className="mt-6 text-[clamp(2.5rem,6vw,4.8rem)] font-black leading-[.98] tracking-[-.05em] text-foreground">Find the right PDF, document or image tool in seconds.</h1><p className="mx-auto mt-5 max-w-2xl text-base font-medium leading-7 text-muted-foreground">Search, filter and choose the layout that fits your screen. Open any tool to see supported files, useful options and important limits before you begin.</p></div>
    <div className="mx-auto mt-9 max-w-2xl"><label htmlFor="directory-search" className="sr-only">Search public tools</label><div className="ajn-glass-card relative rounded-2xl p-2"><Search className="absolute left-6 top-1/2 h-5 w-5 -translate-y-1/2 text-blue-600" /><Input id="directory-search" value={search} onChange={(event)=>changeSearch(event.target.value)} placeholder="Search merge, convert, image, protect…" className="h-14 rounded-xl border-0 bg-transparent pl-12 pr-12 text-foreground shadow-none placeholder:text-muted-foreground focus-visible:ring-blue-500/25" />{search && <button type="button" onClick={()=>changeSearch('')} className="absolute right-5 top-1/2 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-xl text-muted-foreground hover:bg-muted hover:text-foreground" aria-label="Clear tool search"><X className="h-4 w-4" /></button>}</div></div>
    <div className="mt-7 flex gap-2 overflow-x-visible flex-wrap pb-2 scrollbar-hide md:justify-center" aria-label="Tool categories">{categories.map(({id,label,icon:Icon})=><button type="button" key={id} onClick={()=>selectCategory(id)} aria-pressed={activeCategory===id} className={cn('flex min-h-11 shrink-0 items-center gap-2 rounded-xl border px-4 py-3 text-[11px] font-black transition focus-visible:ring-2 focus-visible:ring-blue-500',activeCategory===id?'border-blue-600 bg-blue-600 text-white shadow-lg':'border-border bg-card text-muted-foreground hover:border-blue-300 hover:bg-blue-50')}><Icon className="h-4 w-4" />{label}<span className={cn('rounded-lg px-2 py-0.5 text-[9px]',activeCategory===id?'bg-white/15':'bg-muted text-muted-foreground')}>{counts[id]}</span></button>)}</div>
    <div className="mt-12"><ServicesGrid query={search} category={activeCategory} /></div>
