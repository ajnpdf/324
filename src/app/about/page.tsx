      ...
      <main className="relative z-10 mx-auto max-w-7xl px-4 pb-24 pt-32 md:px-8 md:pt-40">
        <section className="grid items-center gap-12 lg:grid-cols-[1fr_.9fr]">
          <div>
            ...
            <p className="mt-6 max-w-2xl text-lg font-medium leading-8 text-muted-foreground">AJN PDF brings PDF, document, OCR and image workflows into one focused web workspace designed for fast everyday file tasks.</p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Link href="/pdf-tools" className="ajn-primary-button">Explore public tools <ArrowRight className="h-4 w-4" /></Link>
              <Link href="/transparency" className="ajn-secondary-button">File handling details</Link>
            </div>
          </div>
        ...
        <section className="mt-24">
          <div className="max-w-3xl"><span className="ajn-section-kicker">Product principles</span><h2 className="mt-5 text-4xl font-black tracking-[-.04em] text-foreground md:text-6xl">Built around clarity, useful workflows and dependable results.</h2></div>
          <div className="mt-12 grid gap-5 lg:grid-cols-3">
            {principles.map(({ icon: Icon, title, text, tone }) => <article key={title} className="ajn-glass-card rounded-3xl p-7"><span className={`flex h-12 w-12 items-center justify-center rounded-2xl ${tone === 'red' ? 'bg-red-50 text-red-600' : tone === 'green' ? 'bg-emerald-50 text-emerald-600' : 'bg-blue-50 text-blue-600'}`}><Icon className="h-5 w-5" /></span><h3 className="mt-6 text-xl font-black text-foreground">{title}</h3><p className="mt-3 text-sm font-medium leading-6 text-muted-foreground">{text}</p></article>)}
          </div>
        </section>
