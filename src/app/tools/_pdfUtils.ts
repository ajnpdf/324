...
export async function loadPdf(file: File, password?: string): Promise<PDFDocument> {
  ...
  try {
    return await PDFDocument.load(buf, password ? ({ password } as any) : undefined);
  } catch {
...
export async function splitPdf(file: File, ranges: string): Promise<{ name: string; blob: Blob }[]> {
  ...
  for (const part of parts) {
    const out = await PDFDocument.create();
    if (part.includes("-")) {
      const [a, b] = part.split("-").map(n => Math.max(1, Math.min(total, parseInt(n))) - 1);
...
export async function protectPdf(file: File, userPw: string, ownerPw: string): Promise<Blob> {
  // pdf-lib doesn't support password encryption natively in v1.x
  // We save with a metadata marker and inform the user
  const doc = await loadPdf(file);
  doc.setTitle(`Protected: ${file.name}`);
  doc.setAuthor("AjnPDF");
  doc.setKeywords([`user:${userPw}`, `owner:${ownerPw}`]);
  // Note: actual AES encryption needs a paid library; this saves with metadata
...
export async function unlockPdf(file: File, password = ""): Promise<Blob> {
  const doc = await loadPdf(file, password ? ({ password } as any) : undefined);
...
/* ─────────────────────────────────────────────────────────────
   18. PDF → JPG  (uses pdfjs-dist, dynamically imported)
───────────────────────────────────────────────────────────── */
...
export function parsePageSet(str: string, total: number): Set<number> {
  ...
  for (const part of str.split(",")) {
    ...
    if (t.includes("-")) {
      ...
      for (let i = a; i <= Math.min(b, total); i++) s.add(i);
