...
export default function ImageToPdfFormatGuide() {
  return <GuideArticle
    ...
    sections={[
      ...
      { title: 'Image PDFs are not automatically searchable', paragraphs: ['Placing an image into a PDF does not create selectable text. If search or copy is required, run an OCR workflow after creating the PDF or use a searchable-PDF conversion designed for the source images.', 'Keep the original images until you have checked the final PDF.'] },
    ]}
    checklist={['JPG for photographs', 'PNG for sharp text and graphics', 'Choose page fit carefully', 'Use OCR when searchable text is needed']}
    relatedTools={[
      ...
      { href: '/image-to-searchable-pdf', title: 'Image to Searchable PDF', description: 'Create a PDF with OCR text from supported images.' },
    ]}
  />;
}
