...
export default function PdfAccessibilityBasicsGuide() {
  return <GuideArticle
    ...
    sections={[
      { title: 'Start with real text where possible', paragraphs: ['A PDF made only from page images is harder to search, select and navigate with assistive technology. OCR can add machine-readable text to scans, but recognized text still needs review.', 'When creating a new document, exporting from a structured source is usually better than printing everything to an image.'] },
    ...
    relatedTools={[
      { href: '/scanned-pdf-to-searchable-pdf', title: 'Searchable PDF', description: 'Add an OCR text layer to supported scans.' },
      { href: '/pdf-text', title: 'PDF to Text', description: 'Check whether a PDF contains extractable text.' },
      { href: '/ocr', title: 'OCR guide', description: 'Review AJN PDF OCR languages and scan-quality guidance.' },
    ]}
  />;
}
