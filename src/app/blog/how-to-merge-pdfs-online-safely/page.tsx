...
export default function MergePdfSafetyGuide() {
  return <GuideArticle
    ...
    summary="A reliable merge workflow checks processing mode, page order, output quality and the sensitivity of the documents before downloading the result."
    readTime="5 minute guide"
    sections={[
      { title: 'Confirm the processing mode', paragraphs: ['AJN PDF classifies Merge PDF as a browser workflow. The selected PDFs are combined in the current browser session rather than sent to the Python conversion service.', 'Browser processing reduces network transfer, but users should still work on a trusted device and avoid leaving sensitive files open on a shared computer.'], note: 'Use the browser Network panel when you need to independently verify that a local tool does not send the selected file to the AJN backend.' },
