    ...
    try:
        completed = subprocess.run(
            [
                ...
                "--convert-to", filter_name, "--outdir", str(root), str(source),
            ],
...
def fixtures(root: Path) -> dict[str, Path]:
    ...
    pdf_canvas.drawString(72, 730, "English OCR and conversion sample")
    pdf_canvas.save()
    files[".pdf"] = pdf
    ...
    for candidate in ("C:/Windows/Fonts/arial.ttf", "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf"):
        try:
            ...
            continue
    draw.text((70, 140), "AJN PDF OCR TEST 123", fill="black", font=font)
    ...
    svg.write_text(
        ...
        '<text x="20" y="80" font-size="28">AJN PDF SVG</text></svg>',
        encoding="utf-8",
    )
    files[".svg"] = svg
    ...
    markdown.write_text("# AJN PDF\n\nMarkdown conversion sample.", encoding="utf-8")
    files[".md"] = markdown
    files[".markdown"] = markdown

    html = root / "sample.html"
    html.write_text(
        "<!doctype html><html><body><h1>AJN PDF</h1><p>HTML sample.</p></body></html>",
        encoding="utf-8",
    )
    files[".html"] = html
    files[".htm"] = html
    ...
    rtf = root / "sample.rtf"
    rtf.write_text(r"{\rtf1\ansi AJN PDF RTF sample}", encoding="utf-8")
    files[".rtf"] = rtf
    ...
    # Generate real legacy/OpenDocument fixtures through the same LibreOffice family
    # used in production. This prevents available Office tools from silently skipping.
    office_variants = [
        (docx, ".doc", "doc:MS Word 97"),
        (docx, ".odt", "odt"),
        (xlsx, ".xls", "xls:MS Excel 97"),
        (xlsx, ".ods", "ods"),
        (pptx, ".ppt", "ppt:MS PowerPoint 97"),
        (pptx, ".odp", "odp"),
    ]
...
def validate_workdir_cleanup(workdir: Path) -> None:
    leftovers = []
    for pattern in ("ocr-source-*", "ocr-page-*", "tess-page-*", "libreoffice-profile-*"):
...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="ajn-full-acceptance-") as temporary:
        ...
        for tool_id, spec in sorted(SPECS.items()):
            ...
            try:
                ...
                convert(
                    ...
                    inputs,
                    output,
                    {"language": "eng", "dpi": 100, "quality": 70, "grayscale": False},
                    workdir,
                    None,
                )
                validate_output(output, spec.output_extension)
                validate_workdir_cleanup(workdir)
                if tool_id in {"image-to-searchable-pdf", "scanned-pdf-to-searchable-pdf"}:
                    with fitz.open(output) as searchable:
                        searchable_text = "\n".join(page.get_text() for page in searchable).strip()
                    if "AJN PDF" not in searchable_text:
                        raise RuntimeError("The searchable PDF does not contain the expected OCR text layer.")
                passed.append(tool_id)
            except Exception as exc:  # Each failure is preserved in the generated report.
                failed.append((tool_id, f"{type(exc).__name__}: {exc}"))

        report = {
            "passed": passed,
            "skipped": [{"id": tool_id, "reason": reason} for tool_id, reason in skipped],
            "failed": [{"id": tool_id, "reason": reason} for tool_id, reason in failed],
        }
        ...
        print(
            "PASS: full acceptance suite produced valid outputs for "
            f"{len(passed)} available tools; {len(skipped)} dependency/fixture-specific tools were documented as skipped."
        )
