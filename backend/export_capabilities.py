...
TARGETS = [
    ROOT / "src" / "generated" / "backend-capabilities.json",
    ROOT / "public" / "backend-capabilities.json",
]
SCHEMA_VERSION = 2
...
def main() -> None:
    ...
    payload = {
        ...
        "toolCount": len(stable_tools),
        "availableCount": available,
        "unavailableCount": len(stable_tools) - available,
    ...
    print(
        "PASS: deployment capability manifest exported with "
        f"{available}/{len(stable_tools)} available backend tools; fingerprint {fingerprint[:12]}"
    )
