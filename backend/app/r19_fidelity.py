...
def _source_markers(source: Path) -> tuple[list[str], Optional[int]]:
    ...
    try:
        if ext == '.docx':
            ...
            for text in blocks:
                markers.extend(re.findall(r'[A-Za-z0-9][A-Za-z0-9._-]{2,}', text or '')[:4])
        elif ext == '.xlsx':
                    ...
                    for row in ws.iter_rows(values_only=True):
                        for value in row:
                            if value not in (None, ''):
                                markers.extend(re.findall(r'[A-Za-z0-9][A-Za-z0-9._-]{2,}', str(value))[:2])
                                used += 1
                                if used >= 20:
                                    break
                        if used >= 20:
            ...
            for slide in prs.slides[:8]:
                for shape in slide.shapes:
                    if hasattr(shape, 'text'):
                        markers.extend(re.findall(r'[A-Za-z0-9][A-Za-z0-9._-]{2,}', str(shape.text or ''))[:4])
    except Exception:
...
def office_to_pdf_fidelity(source: Path, output: Path, workdir: Path, options: dict[str, Any]) -> None:
    ...
    for attempt in range(1, 3):
        ...
        command = [
            executable, '--headless', '--nologo', '--nodefault', '--nolockcheck', '--norestore',
            f'-env:UserInstallation={profile.resolve().as_uri()}', '--convert-to', _lo_filter(source, options),
    ...
    with fitz.open(source) as pdf:
        ...
        for page_number, page in enumerate(pdf, start=1):
            ...
            for block in page.get_text('dict', sort=True).get('blocks', []):
                ...
                frame.clear(); frame.margin_left = frame.margin_right = frame.margin_top = frame.margin_bottom = 0
                ...
                for line in block.get('lines', []):
                    ...
                    for span in line.get('spans', []):
                        ...
                        font = str(span.get('font') or '').lower(); flags = int(span.get('flags') or 0)
            ...
            for info in page.get_images(full=True):
                ...
                seen.add(xref)
                try:
                    extracted = pdf.extract_image(xref); data = extracted.get('image')
                    if not data:
                        continue
...
def _ocr_page_text(page: fitz.Page, options: dict[str, Any]) -> str:
    dpi = max(180, min(350, int(options.get('dpi', 240))))
    language = str(options.get('language', 'eng') or 'eng')
    pix = page.get_pixmap(matrix=fitz.Matrix(dpi/72.0, dpi/72.0), alpha=False)
    image = Image.frombytes('RGB', (pix.width, pix.height), pix.samples)
    try:
        return pytesseract.image_to_string(image, lang=language, config='--oem 1 --psm 3').strip()
    finally:
        image.close()
...
def pdf_to_docx_fidelity(source: Path, output: Path, options: dict[str, Any], workdir: Path) -> None:
    ...
    # Remove the empty default paragraph after we start writing the first real item.
    with fitz.open(source) as pdf:
        if pdf.page_count < 1:
            raise ValueError('The PDF has no pages.')
        for page_index, page in enumerate(pdf):
            ...
            if len(re.sub(r'\s+', '', native_text)) < 24:
                ocr = _ocr_page_text(page, options)
                if not ocr:
    ...
    # Drop the initial empty paragraph when it is still empty and there is later content.
...
def _coerce(value: str) -> Any:
    ...
    if re.fullmatch(r'[-+]?\d*\.\d+', normalized):
        try: return float(normalized)
        except Exception: pass
    currency = re.fullmatch(r'[₹$€£]\s*([-+]?\d+(?:,\d{3})*(?:\.\d+)?)', text)
    if currency:
        try: return float(currency.group(1).replace(',', ''))
        except Exception: pass
    return text
...
def _ocr_tables(source: Path, options: dict[str, Any]) -> list[tuple[int, int, list[list[str]]]]:
    language = str(options.get('language', 'eng') or 'eng')
    dpi = max(180, min(350, int(options.get('dpi', 240))))
    found: list[tuple[int, int, list[list[str]]]] = []
    with fitz.open(source) as pdf:
        for page_number, page in enumerate(pdf, start=1):
            ...
            image = Image.frombytes('RGB', (pix.width, pix.height), pix.samples)
            try:
                data = pytesseract.image_to_data(image, lang=language, config='--oem 1 --psm 6', output_type=pytesseract.Output.DICT)
            finally:
                image.close()
            heights: list[int] = []; lines: dict[tuple[int,int,int], list[tuple[int,int,str]]] = {}
            for i, raw in enumerate(data.get('text', [])):
                token = str(raw or '').strip()
                if not token: continue
                key = (int(data['block_num'][i]), int(data['par_num'][i]), int(data['line_num'][i]))
                left = int(data['left'][i]); width = int(data['width'][i]); heights.append(max(1, int(data['height'][i])))
                lines.setdefault(key, []).append((left, width, token))
            threshold = max(30, int((statistics.median(heights) if heights else 20) * 2.4))
            rows: list[list[str]] = []
            for words in lines.values():
                words.sort(key=lambda item: item[0]); row: list[str] = []; current: list[str] = []; last_right: Optional[int] = None
                for left, word_width, token in words:
                    if last_right is not None and left - last_right > threshold and current:
                        row.append(' '.join(current)); current = []
                    current.append(token); last_right = left + word_width
                if current: row.append(' '.join(current))
                if len(row) >= 2: rows.append(row)
            if len(rows) >= 2: found.append((page_number, 1, rows))
    return found
    ...
    for page_number, table_number, rows in tables:
        ...
        for ri, row in enumerate(rows, start=1):
            for ci, value in enumerate(row, start=1):
                cell = ws.cell(ri, ci, _coerce(value)); cell.alignment = Alignment(vertical='top', wrap_text=True)
                if ri == 1:
                    cell.font = Font(bold=True); cell.fill = PatternFill('solid', fgColor='E8EEF8')
...
def pdf_to_xlsx_fidelity(source: Path, output: Path, options: dict[str, Any]) -> None:
    ...
    if not tables and bool(options.get('scanned_table_fallback', False)):
        tables = _ocr_tables(source, options)
    if not tables:
        if bool(options.get('allow_unstructured', False)):
            wb = Workbook(); ws = wb.active; ws.title = 'PDF text'; ws.append(['Page','Line','Text'])
            with fitz.open(source) as pdf:
                for page_number, page in enumerate(pdf, start=1):
                    for line_number, line in enumerate(page.get_text('text', sort=True).splitlines(), start=1):
                        if line.strip(): ws.append([page_number, line_number, line.strip()])
            wb.save(output); return
...
def pdf_to_csv_fidelity(source: Path, output: Path, options: dict[str, Any]) -> None:
    ...
    with output.open('w', encoding='utf-8-sig', newline='') as handle:
        ...
        for index, (page_number, table_number, rows) in enumerate(tables):
            ...
            writer.writerow([f'Page {page_number} - Table {table_number}']); writer.writerows(rows)
...
def run_r19_conversion(spec: legacy.ConversionSpec, files: list[Path], output: Path, options: dict[str, Any], workdir: Path, source_url: Optional[str] = None) -> bool:
    ...
    if spec.processor == 'office_to_pdf':
        office_to_pdf_fidelity(source, output, workdir, options); return True
    if spec.processor == 'pdf_docx':
        pdf_to_docx_fidelity(source, output, options, workdir); return True
    if spec.processor == 'pdf_pptx':
        pdf_to_pptx_fidelity(source, output, options, workdir); return True
    if spec.processor == 'pdf_xlsx':
        pdf_to_xlsx_fidelity(source, output, options); return True
    if spec.processor == 'pdf_csv':
        pdf_to_csv_fidelity(source, output, options); return True
    return False
