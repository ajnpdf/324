...
MIN_USEFUL_OCR_CHARS = 3
DEFAULT_OCR_DPI = 240
MAX_TABLES_PER_DOCUMENT = 64
MAX_DOCX_IMAGES = 64


def _ocr_languages(options: dict[str, Any]) -> str:
    ...
    missing = sorted({item for item in requested.split("+") if item}.difference(installed))
    if missing:
...
def _prepare_ocr_image(image: Image.Image, options: dict[str, Any], *, preserve_color: bool = False) -> Image.Image:
...
def _ocr_image_model(image: Image.Image, options: dict[str, Any], *, handwriting: bool = False) -> dict[str, Any]:
    language = _ocr_languages(options)
    executable = legacy.command_path("tesseract")
    if executable:
        pytesseract.pytesseract.tesseract_cmd = executable
    prepared = _prepare_ocr_image(image, options)
    try:
        psm = int(options.get("psm", 6 if handwriting else 3))
        data = pytesseract.image_to_data(
            prepared,
            lang=language,
            config=f"--oem 1 --psm {max(3, min(13, psm))} -c preserve_interword_spaces=1",
    ...
    lines = [" ".join(words).strip() for _, words in sorted(grouped.items()) if words]
    text = "\n".join(lines).strip()
    confidence = round(sum(confidences) / len(confidences), 1) if confidences else 0.0
    if len(re.sub(r"\s+", "", text)) < MIN_USEFUL_OCR_CHARS and confidence < 20:
        raise ValueError("OCR could not recognize useful text. Try a clearer, straighter, higher-resolution scan.")
    return {"text": text, "lines": lines, "confidence": confidence, "source": "ocr"}
...
def _extract_pdf_models(source: Path, options: dict[str, Any], *, force_ocr: bool = False) -> list[dict[str, Any]]:
    ...
    with fitz.open(source) as document:
        ...
        for index, page in enumerate(document, start=1):
            ...
            useful_native = len(re.sub(r"\s+", "", native["text"])) >= MIN_NATIVE_TEXT_CHARS
            if force_ocr or not useful_native:
                if legacy.command_path("tesseract"):
                    rendered = _render_page(page, dpi)
                    try:
                        ocr = _ocr_image_model(rendered, options)
                    finally:
                        rendered.close()
                    model = {**native, **ocr, "blocks": native.get("blocks", []), "page": index}
                else:
...
def _write_text(models: list[dict[str, Any]], output: Path) -> None:
    ...
    if not useful:
        raise ValueError("No readable text was found in this PDF. Try OCR with the correct language selected.")
    output.write_text(
        "\n\n".join(f"--- Page {model['page']} ---\n{str(model.get('text') or '').strip()}" for model in models),
        encoding="utf-8",
    )
        ...
        for file in files:
            for frame in _iter_input_frames(file):
                try:
                    ...
                    image = _prepare_ocr_image(frame, options, preserve_color=not bool(options.get("grayscale", scan))) if scan else frame.convert("RGB")
                    try:
...
def _searchable_from_images(files: list[Path], output: Path, options: dict[str, Any], workdir: Path) -> None:
    ...
    try:
        for file in files:
            for frame in _iter_input_frames(file):
                page_index += 1
                prepared = _prepare_ocr_image(frame, options, preserve_color=True)
                ...
                try:
                    ...
                    legacy._tesseract_searchable_pdf(executable, source_path, part, language, workdir, page_index)
                    parts.append(part)
                finally:
                    frame.close()
                    prepared.close()
...
def run_conversion(
    ...
    """Quality-first website conversion entrypoint.

    The public FastAPI service executes conversions through ``job_worker``. This wrapper
    upgrades the user-facing OCR/PDF/image paths while retaining the proven legacy
    processors for formats where the existing implementation is already the safer option.
    MCP code is intentionally not changed by this layer.
    """
    ...
    if processor == "pdf_text":
    ...
    elif processor == "ocr_pdf_text":
        _write_text(_extract_pdf_models(source, options, force_ocr=True), output)
    elif processor == "ocr_pdf_word":
        models = _extract_pdf_models(source, options, force_ocr=True)
        _write_layout_docx(source, models, output, {**options, "include_images": False})
    elif processor == "ocr_pdf_searchable":
        _searchable_from_pdf(source, output, options, workdir)
        ...
        for file in files:
            for frame in _iter_input_frames(file):
                page_number += 1
                try:
                    model = _ocr_image_model(frame, options, handwriting=processor == "ocr_handwriting_text")
                finally:
                    frame.close()
                models.append({**model, "page": page_number})
        if processor == "ocr_image_word":
            document = Document()
            document.core_properties.title = files[0].stem if files else "AJN OCR"
            ...
            _write_text(models, output)
    elif processor == "ocr_image_searchable":
        _searchable_from_images(files, output, options, workdir)
    else:
        legacy.convert(spec, files, output, options, workdir, source_url)
        # Legacy convert validates most outputs itself. The explicit validation here also
        # closes historical early-return paths such as XPS -> PDF.
        legacy.validate_output_file(output, spec.output_extension)
        return
