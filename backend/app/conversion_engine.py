...
MAX_IMAGE_PIXELS = max(10_000_000, int(os.getenv("AJN_MAX_IMAGE_PIXELS", "80000000")))
MAX_IMAGE_FRAMES = max(1, int(os.getenv("AJN_MAX_IMAGE_FRAMES", "120")))
MAX_BATCH_PIXELS = max(50_000_000, int(os.getenv("AJN_MAX_BATCH_MPIX", "240")) * 1_000_000)
MAX_PDF_PAGES = max(1, int(os.getenv("AJN_MAX_PDF_PAGES", "300")))
MAX_RENDER_PIXELS = max(50_000_000, int(os.getenv("AJN_MAX_RENDER_MPIX", "600")) * 1_000_000)
...
# OCR and scan workflows
for tool_id, name, out_ext, out_mime, processor, limitation in [
    ("scanned-pdf-to-text", "Scanned PDF to Text", ".txt", "text/plain", "ocr_pdf_text", "OCR accuracy depends on scan quality, language and page layout."),
    ("scanned-pdf-to-word", "Scanned PDF to Word", ".docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "ocr_pdf_word", "The result is editable text and does not recreate every original layout element."),
    ("scanned-pdf-to-searchable-pdf", "Scanned PDF to Searchable PDF", ".pdf", "application/pdf", "ocr_pdf_searchable", "The searchable layer is generated from OCR and should be reviewed for accuracy."),
    ("image-to-searchable-pdf", "Image to Searchable PDF", ".pdf", "application/pdf", "ocr_image_searchable", "OCR accuracy depends on image quality and selected language."),
    ("image-to-text", "Image to Text", ".txt", "text/plain", "ocr_image_text", "OCR accuracy depends on image quality and selected language."),
    ("image-to-word", "Image to Word", ".docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "ocr_image_word", "The Word file contains recognized text, not a pixel-perfect recreation."),
    ("camera-scan-to-pdf", "Camera Scan to PDF", ".pdf", "application/pdf", "scan_images_pdf", "Automatic cleanup improves readability but may not correct every perspective issue."),
    ("receipt-to-pdf", "Receipt to PDF", ".pdf", "application/pdf", "scan_images_pdf", "Receipt cleanup is optimized for contrast and readability."),
    ("document-scanner-to-pdf", "Document Scanner to PDF", ".pdf", "application/pdf", "scan_images_pdf", "Automatic cleanup is best for clear, well-lit document photos."),
    ("handwriting-image-to-text", "Handwriting Image to Text", ".txt", "text/plain", "ocr_handwriting_text", "Handwriting recognition is best-effort and usually less accurate than printed text OCR."),
]:
    _add(ConversionSpec(tool_id, name, "ocr", (".pdf",) if tool_id.startswith("scanned-pdf") else (".jpg", ".jpeg", ".png", ".webp", ".tif", ".tiff", ".bmp", ".gif", ".heic", ".heif"), out_ext, out_mime, processor, multi_file=tool_id in {"image-to-searchable-pdf", "camera-scan-to-pdf", "receipt-to-pdf", "document-scanner-to-pdf"}, limitation=limitation, external_dependency="tesseract" if processor.startswith("ocr_") else None))

# Image -> PDF
for ext, slug, label in [
    ("*", "image-to-pdf", "Image to PDF"),
    (".jpg", "jpg-to-pdf", "JPG to PDF"),
    (".jpeg", "jpeg-to-pdf", "JPEG to PDF"),
    (".png", "png-to-pdf", "PNG to PDF"),
    (".webp", "webp-to-pdf", "WEBP to PDF"),
    (".tiff", "tiff-to-pdf", "TIFF to PDF"),
    (".bmp", "bmp-to-pdf", "BMP to PDF"),
    (".gif", "gif-to-pdf", "GIF to PDF"),
    (".svg", "svg-to-pdf", "SVG to PDF"),
    (".heic", "heic-to-pdf", "HEIC to PDF"),
]:
...
# PDF -> image
for slug, label, fmt, limitation in [
    ("pdf-to-image", "PDF to Image", "png", None),
    ("pdf-to-jpg", "PDF to JPG", "jpg", None),
    ("pdf-to-jpeg", "PDF to JPEG", "jpeg", None),
    ("pdf-to-png", "PDF to PNG", "png", None),
    ("pdf-to-webp", "PDF to WEBP", "webp", None),
    ("pdf-to-tiff", "PDF to TIFF", "tiff", None),
    ("pdf-to-bmp", "PDF to BMP", "bmp", None),
    ("pdf-to-gif", "PDF to GIF", "gif", "Multiple pages are returned as an animated GIF."),
    ("pdf-to-svg", "PDF to SVG", "svg", "Text and vector fidelity depends on the PDF page content."),
    ("pdf-to-avif", "PDF to AVIF", "avif", "AVIF encoding availability depends on the installed Pillow build."),
...
# PDF -> document
pdf_document_specs = [
    ("pdf-to-word", "PDF to Word", ".docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "pdf_docx", "Creates an editable text-focused document; complex layout may change."),
    ("pdf-to-docx", "PDF to DOCX", ".docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "pdf_docx", "Creates an editable text-focused document; complex layout may change."),
    ("pdf-to-txt", "PDF to TXT", ".txt", "text/plain", "pdf_text", None),
    ("pdf-to-rtf", "PDF to RTF", ".rtf", "application/rtf", "pdf_rtf", "Preserves text content rather than exact page design."),
    ("pdf-to-odt", "PDF to ODT", ".odt", "application/vnd.oasis.opendocument.text", "pdf_odt", "Creates an editable text-focused document; complex layout may change."),
    ("pdf-to-html", "PDF to HTML", ".html", "text/html", "pdf_html", "Creates readable HTML from extracted text; advanced PDF positioning may change."),
    ("pdf-to-markdown", "PDF to Markdown", ".md", "text/markdown", "pdf_markdown", "Headings and tables are inferred from extracted text and may require editing."),
    ("pdf-to-xml", "PDF to XML", ".xml", "application/xml", "pdf_xml", None),
    ("pdf-to-json", "PDF to JSON", ".json", "application/json", "pdf_json", None),
    ("pdf-to-csv", "PDF to CSV", ".csv", "text/csv", "pdf_csv", "Rows are created from extracted text lines; this is not advanced table recognition."),
    ("pdf-to-excel", "PDF to Excel", ".xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "pdf_xlsx", "Rows are created from extracted text lines; complex tables may require correction."),
    ("pdf-to-xlsx", "PDF to XLSX", ".xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "pdf_xlsx", "Rows are created from extracted text lines; complex tables may require correction."),
    ("pdf-to-powerpoint", "PDF to PowerPoint", ".pptx", "application/vnd.openxmlformats-officedocument.presentationml.presentation", "pdf_pptx", "Each PDF page is placed as a slide image to preserve appearance."),
    ("pdf-to-pptx", "PDF to PPTX", ".pptx", "application/vnd.openxmlformats-officedocument.presentationml.presentation", "pdf_pptx", "Each PDF page is placed as a slide image to preserve appearance."),
    ("pdf-to-epub", "PDF to EPUB", ".epub", "application/epub+zip", "pdf_epub", "Creates a reflowable text-focused eBook; complex page design may change."),
    ("pdf-to-mobi", "PDF to MOBI", ".mobi", "application/x-mobipocket-ebook", "pdf_ebook_external:mobi", "Requires Calibre ebook-convert and creates a text-focused eBook."),
    ("pdf-to-azw3", "PDF to AZW3", ".azw3", "application/vnd.amazon.ebook", "pdf_ebook_external:azw3", "Requires Calibre ebook-convert and creates a text-focused eBook."),
]
...
# Document -> PDF
for slug, label, exts, processor, limitation, dep in [
    ("word-to-pdf", "Word to PDF", (".doc", ".docx"), "office_to_pdf", "Complex fonts, macros and advanced layouts may render differently.", "libreoffice"),
    ("doc-to-pdf", "DOC to PDF", (".doc",), "office_to_pdf", "Requires LibreOffice for legacy DOC conversion.", "libreoffice"),
    ("docx-to-pdf", "DOCX to PDF", (".docx",), "office_to_pdf", "Complex fonts and advanced layouts may render differently.", "libreoffice"),
    ("txt-to-pdf", "TXT to PDF", (".txt",), "text_document_to_pdf", None, None),
    ("rtf-to-pdf", "RTF to PDF", (".rtf",), "rtf_to_pdf", "Rich formatting is simplified for a consistent PDF output.", None),
    ("odt-to-pdf", "ODT to PDF", (".odt",), "office_to_pdf", "Requires LibreOffice.", "libreoffice"),
    ("ods-to-pdf", "ODS to PDF", (".ods",), "office_to_pdf", "Requires LibreOffice; print areas and charts may render differently.", "libreoffice"),
    ("odp-to-pdf", "ODP to PDF", (".odp",), "office_to_pdf", "Requires LibreOffice; animations and transitions are not included.", "libreoffice"),
    ("html-to-pdf", "HTML to PDF", (".html", ".htm"), "html_file_to_pdf", "External scripts and complex browser-only CSS are not executed.", None),
    ("url-to-pdf", "URL to PDF", tuple(), "url_to_pdf", "Creates a readable snapshot; JavaScript-heavy pages may not match the browser exactly.", None),
    ("markdown-to-pdf", "Markdown to PDF", (".md", ".markdown"), "markdown_to_pdf", None, None),
    ("xml-to-pdf", "XML to PDF", (".xml",), "structured_text_to_pdf", None, None),
    ("json-to-pdf", "JSON to PDF", (".json",), "structured_text_to_pdf", None, None),
    ("csv-to-pdf", "CSV to PDF", (".csv",), "structured_text_to_pdf", None, None),
    ("excel-to-pdf", "Excel to PDF", (".xls", ".xlsx"), "office_to_pdf", "Requires LibreOffice; print areas and charts may render differently.", "libreoffice"),
    ("xls-to-pdf", "XLS to PDF", (".xls",), "office_to_pdf", "Requires LibreOffice.", "libreoffice"),
    ...
    ("powerpoint-to-pdf", "PowerPoint to PDF", (".ppt", ".pptx"), "office_to_pdf", "Requires LibreOffice; animations and transitions are not included.", "libreoffice"),
    ("ppt-to-pdf", "PPT to PDF", (".ppt",), "office_to_pdf", "Requires LibreOffice.", "libreoffice"),
    ("pptx-to-pdf", "PPTX to PDF", (".pptx",), "office_to_pdf", "Requires LibreOffice; animations and transitions are not included.", "libreoffice"),
    ("epub-to-pdf", "EPUB to PDF", (".epub",), "ebook_to_pdf", "Creates a printable layout from reflowable eBook content.", None),
    ("mobi-to-pdf", "MOBI to PDF", (".mobi",), "ebook_external_to_pdf", "Requires Calibre ebook-convert.", "calibre"),
...
def command_path(name: str) -> str | None:
    # Windows must use soffice.exe, never the soffice.COM console wrapper. The
    # .COM wrapper can remain attached to a hidden LibreOffice process and has
    # caused conversion timeouts in production acceptance runs.
    ...
    candidates = {
        ...
        "mutool": ["mutool", "gxps", "gswin64c", "gs"],
        "tesseract": ["tesseract"],
    }.get(name, [name])
    for candidate in candidates:
...
def available_ocr_languages() -> list[str]:
    ...
    try:
        ...
        return sorted(language for language in languages if not language.lower().startswith("list of available"))
    except Exception:
        return ["eng"]
...
def tool_available(spec: ConversionSpec) -> tuple[bool, str | None]:
    ...
    if spec.processor.startswith("ocr_") and not command_path("tesseract"):
        return False, "Tesseract OCR is required on the processing server."
        ...
        result.append(
            {
                ...
                "inputExtensions": list(spec.input_extensions),
                "outputExtension": spec.output_extension,
                "available": available,
                "unavailableReason": reason,
                "limitation": spec.limitation,
                "multiFile": spec.multi_file,
                "ocrLanguages": available_ocr_languages() if spec.processor.startswith("ocr_") else [],
...
def list_backend_tools() -> list[dict[str, Any]]:
    """Return every backend-dependent public capability used by the frontend build.

    Protect, Unlock and Repair are intentionally separate API endpoints rather than
    ConversionSpec entries, so they must be exported here as first-class capabilities.
    Without these records the dependency-aware frontend build would incorrectly hide
    those tools even when the backend is healthy.
    """
    tools = list_tools()
    tools.extend(
        [
            {
                ...
                "limitation": "Applies AES-256 encryption. Keep the password securely; AJN PDF cannot recover it.",
                "multiFile": False,
                "ocrLanguages": [],
                "processingMode": "temporary-server",
            },
            {
                ...
                "available": True,
                "unavailableReason": None,
                "limitation": "Requires the current valid PDF password and user authorization.",
                "multiFile": False,
                "ocrLanguages": [],
                "processingMode": "temporary-server",
            },
            {
                ...
                "available": True,
                "unavailableReason": None,
                "limitation": "Attempts structural recovery of readable PDF content; severe corruption may be unrecoverable.",
                "multiFile": False,
                "ocrLanguages": [],
                "processingMode": "temporary-server",
            },
...
_ZIP_BASED_EXPECTATIONS: dict[str, tuple[str, ...]] = {
    ".docx": ("word/document.xml",),
    ".xlsx": ("xl/workbook.xml",),
    ".pptx": ("ppt/presentation.xml",),
    ".odt": ("content.xml", "mimetype"),
    ".ods": ("content.xml", "mimetype"),
    ".odp": ("content.xml", "mimetype"),
...
def validate_input_file(path: Path, allow_encrypted_pdf: bool = False) -> None:
    """Validate that an uploaded file structurally matches its claimed extension.

    This intentionally uses conservative, format-aware checks before native/third-party
    converters receive untrusted data. It is not a malware scanner, but it prevents the
    common case of renamed or obviously damaged inputs reaching LibreOffice, Pillow,
    PyMuPDF, Tesseract, Calibre or message parsers.
    """
    ...
    if ext == ".svg":
        try:
            ...
            for match in re.findall(r"url\(([^)]+)\)", raw_svg, flags=re.I):
                value = match.strip().strip("\"'")
                if value and not value.startswith("#") and not value.lower().startswith("data:"):
                    raise ValueError("External SVG resources are not allowed.")
            if re.search(r"@import\s+(?:url\()?\s*[\"']?(?!data:|#)", raw_svg, flags=re.I):
                raise ValueError("External SVG resources are not allowed.")
        except ValueError:
            raise
        except Exception as exc:
...
def _render_pdf_pages(pdf_path: Path, dpi: int = 150) -> list[Image.Image]:
    # Retained only for output formats that genuinely require all frames at once
    # (animated GIF and multipage TIFF). Apply a stricter in-memory batch ceiling.
    images: list[Image.Image] = []
    total_pixels = 0
    try:
...
def _office_to_pdf(source: Path, output: Path, workdir: Path) -> None:
    ...
    for attempt in range(1, 3):
        ...
        try:
            ...
            # Some Windows LibreOffice builds return before the output handle is flushed.
            previous_size = -1
            stable_samples = 0
            for _ in range(60):
            ...
            last_error = exc
            # Retry once with a fresh isolated LibreOffice profile.
            if attempt == 1:
                time.sleep(1)
                continue
            break
...
def _ocr_image(image: Image.Image, language: str, handwriting: bool = False) -> str:
    executable = command_path("tesseract")
    if executable:
        pytesseract.pytesseract.tesseract_cmd = executable
    config = "--oem 1 --psm 6" if handwriting else "--oem 1 --psm 3"
    return pytesseract.image_to_string(_preprocess_scan(image), lang=language, config=config).strip()


def _ocr_pdf_pages(source: Path, options: dict[str, Any]) -> list[dict[str, Any]]:
    language = str(options.get("language", "eng"))
    dpi = max(150, min(400, int(options.get("dpi", 220))))
    pages = []
    for index, image in _iter_pdf_rendered_pages(source, dpi):
        try:
...
def _ocr_images(files: list[Path], options: dict[str, Any], handwriting: bool = False) -> list[dict[str, Any]]:
    language = str(options.get("language", "eng"))
    pages = []
    page_number = 0
    for file in files:
        for image in _iter_frames(file):
            page_number += 1
            text = _ocr_image(image, language, handwriting)
            pages.append({"page": page_number, "text": text, "lines": [line for line in text.splitlines() if line.strip()]})
            image.close()
    return pages
...
def _validate_generated_pdf_bytes(pdf_bytes: bytes) -> None:
    if len(pdf_bytes) < 64 or not pdf_bytes.startswith(b"%PDF-"):
        raise RuntimeError("Tesseract returned invalid searchable PDF data.")
    try:
        with fitz.open(stream=pdf_bytes, filetype="pdf") as generated:
            if generated.page_count != 1:
                raise RuntimeError("Tesseract returned an invalid searchable PDF page count.")
            # Loading the first page catches truncated xref/stream output before merge.
            generated.load_page(0)
    except RuntimeError:
        raise
    except Exception as exc:
        raise RuntimeError("Tesseract returned a damaged searchable PDF page.") from exc


def _tesseract_searchable_pdf(
    executable: str,
    source_path: Path,
    output_path: Path,
    language: str,
    ...
    """Create one searchable PDF page without relying on Tesseract's `pdf` config file.

    Some Windows Tesseract installs do not ship the optional tessdata/configs/pdf file,
    which previously produced `read_params_file: Can't open pdf`. We explicitly set the
    PDF renderer variables instead. A normal output-base file is preferred because it is
    the most reliable Windows path; binary stdout is retained as a fallback.
    """
    output_base = workdir / f"tess-page-{page_index:03d}"
    generated_path = Path(f"{output_base}.pdf")
    common = [
        "-l", language,
        "--oem", "1",
        "--psm", "3",
    ...
    try:
        ...
        completed = subprocess.run(
            [executable, str(source_path), str(output_base), *common],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            timeout=180,
            check=False,
        )
        if completed.returncode == 0 and generated_path.exists() and generated_path.stat().st_size >= 64:
            pdf_bytes = generated_path.read_bytes()
            _validate_generated_pdf_bytes(pdf_bytes)
            output_path.write_bytes(pdf_bytes)
            return
        detail = completed.stderr.decode("utf-8", errors="ignore")[-1200:].strip()
        errors.append(detail or f"file-output mode exited with {completed.returncode}")
    except subprocess.TimeoutExpired as exc:
        errors.append("file-output mode timed out after 180 seconds")
    except Exception as exc:
        errors.append(f"file-output mode failed: {exc}")
    finally:
        generated_path.unlink(missing_ok=True)
        # Tesseract can leave sidecar files if a renderer variable is ignored.
        for sidecar in workdir.glob(f"{output_base.name}.*"):
            if sidecar != output_path:
                sidecar.unlink(missing_ok=True)

    try:
        completed = subprocess.run(
            [executable, str(source_path), "stdout", *common],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            timeout=180,
            check=False,
        ...
        if completed.returncode != 0:
            detail = completed.stderr.decode("utf-8", errors="ignore")[-1200:].strip()
            errors.append(detail or f"stdout mode exited with {completed.returncode}")
        else:
        ...
        errors.append("stdout mode timed out after 180 seconds")
    except Exception as exc:
        errors.append(f"stdout mode failed: {exc}")

    unique_errors = [item for index, item in enumerate(errors) if item and item not in errors[:index]]
    detail = " | ".join(unique_errors[-3:])
    raise RuntimeError(f"Tesseract could not create a searchable PDF page. {detail}".strip())


def _ocr_searchable_pdf_from_images(files: list[Path], output: Path, options: dict[str, Any], workdir: Path) -> None:
    language = str(options.get("language", "eng")).strip() or "eng"
    executable = command_path("tesseract")
    if not executable:
        raise RuntimeError("Tesseract OCR is not installed on the processing server.")

    installed_languages = set(available_ocr_languages())
    requested_languages = {item for item in language.split("+") if item}
    missing_languages = sorted(requested_languages.difference(installed_languages)) if installed_languages else []
    if missing_languages:
        raise ValueError(f"OCR language data is not installed: {', '.join(missing_languages)}")
    ...
    try:
        for file in files:
            for image in _iter_frames(file):
                ...
                source_path = workdir / f"ocr-source-{page_index:03d}.png"
                part = workdir / f"ocr-page-{page_index:03d}.pdf"
                try:
                    processed = _preprocess_scan(image)
                    processed.save(source_path, "PNG", optimize=True)
                    _tesseract_searchable_pdf(executable, source_path, part, language, workdir, page_index)
                    parts.append(part)
                finally:
                    image.close()
        ...
        try:
            ...
            if merged.page_count < 1:
                raise RuntimeError("The searchable PDF contains no pages.")
            merged.save(output, garbage=4, deflate=True, clean=True)
        finally:
            merged.close()

        if not output.exists() or output.stat().st_size < 64:
            raise RuntimeError("The searchable PDF output was not created correctly.")
        with fitz.open(output) as final_pdf:
            if final_pdf.page_count != page_index:
                raise RuntimeError("The searchable PDF page count does not match the source pages.")
    finally:
        for part in parts:
...
def convert(spec: ConversionSpec, files: list[Path], output: Path, options: dict[str, Any], workdir: Path, source_url: str | None = None) -> None:
    ...
    if processor == "images_to_pdf":
        ...
        if processor == "pdf_text":
            ...
            body = "\\par\n".join(re.sub(r"([\\{}])", r"\\\1", p["text"]) for p in pages)
            output.write_text("{\\rtf1\\ansi\n" + body + "\n}", encoding="utf-8")
        ...
        if not source_url or not re.match(r"^https?://", source_url, flags=re.I):
    ...
    elif processor in {"ocr_pdf_text", "ocr_pdf_word", "ocr_pdf_searchable"}:
        if processor == "ocr_pdf_searchable":
            image_paths = []
            for index, image in _iter_pdf_rendered_pages(source, int(options.get("dpi", 220))):
                path = workdir / f"scan-{index:03d}.png"
                try:
                    image.save(path, "PNG")
                finally:
                    image.close()
                image_paths.append(path)
            _ocr_searchable_pdf_from_images(image_paths, output, options, workdir)
        else:
            pages = _ocr_pdf_pages(source, options)
            if processor == "ocr_pdf_text":
                output.write_text("\n\n".join(f"--- Page {p['page']} ---\n{p['text']}" for p in pages), encoding="utf-8")
            else:
                _write_docx(pages, output, source.stem)
    elif processor in {"ocr_image_text", "ocr_handwriting_text", "ocr_image_word"}:
        pages = _ocr_images(files, options, handwriting=processor == "ocr_handwriting_text")
        if processor == "ocr_image_word":
            _write_docx(pages, output, files[0].stem)
        else:
            output.write_text("\n\n".join(f"--- Image {p['page']} ---\n{p['text']}" for p in pages), encoding="utf-8")
    elif processor == "ocr_image_searchable":
        _ocr_searchable_pdf_from_images(files, output, options, workdir)
    else:
