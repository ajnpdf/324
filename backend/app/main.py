...
logging.basicConfig(
    level=os.getenv("AJN_LOG_LEVEL", "INFO").upper(),
    format="%(asctime)s %(levelname)s %(name)s %(message)s",
)
logger = logging.getLogger("ajn_pdf_api")

app = FastAPI(
    title="AJN PDF Conversion API",
    version=VERSION,
    description="Temporary PDF, OCR, image and document conversion service with automatic cleanup.",
)
app.add_middleware(
    CORSMiddleware,
...
def _http_error_code(status_code: int, detail: str) -> str:
    text = detail.lower()
    if status_code == 413 or "too large" in text or "too many pages" in text or "too many image" in text: return "FILE_TOO_LARGE"
    if status_code == 415: return "UNSUPPORTED_FORMAT"
    if "password" in text: return "WRONG_PASSWORD"
    if "corrupt" in text or "damaged" in text: return "CORRUPT_FILE"
    if status_code == 429: return "RATE_LIMITED"
    if status_code == 504: return "TIMEOUT"
    if status_code == 503: return "SERVICE_UNAVAILABLE"
    if status_code == 400: return "INVALID_REQUEST"
    if status_code == 422: return "PROCESSING_FAILED"
...
async def production_middleware(request: Request, call_next: Callable):
    ...
    try:
        ...
        response = JSONResponse(
            ...
            content={"error": exc.detail, "code": _http_error_code(exc.status_code, str(exc.detail)), "request_id": request_id},
            headers=exc.headers,
        )
    except Exception:
        ...
        response = JSONResponse(
            status_code=500,
            content={
                "error": "The processing service encountered an unexpected error.",
...
async def _save_upload(upload: UploadFile, target: Path, max_bytes: int = MAX_FILE_BYTES, limit_detail: str = "A file exceeds the configured file-size limit.") -> int:
    ...
    UploadFile may already spool large request bodies to disk; copying in bounded chunks
    avoids materialising a second full-size bytes object in application memory.
    """
    total = 0
...
def _file_response(tmp: tempfile.TemporaryDirectory[str], path: Path, download_name: str, media_type: str, tool_id: str) -> FileResponse:
    return FileResponse(
        ...
        headers={"X-AJN-Temporary-Processing": "true", "X-AJN-Tool-ID": tool_id, "X-AJN-Worker-Isolation": "process"},
    )
...
def _record_event_sync(tool_id: str, status: str, duration_ms: int, input_bytes: int, output_bytes: int) -> None:
    ...
    with sqlite3.connect(ANALYTICS_DB, timeout=10) as db:
        db.execute(
            "INSERT INTO conversion_events(tool_id, status, duration_ms, input_bytes, output_bytes) VALUES (?, ?, ?, ?, ?)",
            (tool_id, status, duration_ms, input_bytes, output_bytes),
        )
        db.commit()
...
async def _record_event(tool_id: str, status: str, duration_ms: int, input_bytes: int, output_bytes: int) -> None:
    ...
    try:
        ...
        # Anonymous analytics must never change a conversion result or mask the real error.
    ...
    with sqlite3.connect(ANALYTICS_DB, timeout=10) as db:
        db.execute(
            ...
            INSERT INTO site_events(
              event_name, path, tool_id, metric_name, metric_value, metric_rating,
              element_id, category, query_length_bucket, referrer_group,
              device_type, viewport_bucket, connection_type, theme
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                event.event_name[:64],
                event.path[:300],
                (event.tool_id or "")[:120] or None,
                (event.metric_name or "")[:40] or None,
                event.metric_value,
                (event.metric_rating or "")[:20] or None,
                (event.element_id or "")[:100] or None,
                (event.category or "")[:40] or None,
                (event.query_length_bucket or "")[:30] or None,
                (event.referrer_group or "")[:30] or None,
                (event.device_type or "")[:20] or None,
                (event.viewport_bucket or "")[:20] or None,
                (event.connection_type or "")[:20] or None,
                (event.theme or "")[:12] or None,
            ),
        )
        db.commit()
...
def _health() -> HealthResponse:
    ...
    return HealthResponse(
        status="ok",
        service="ajn-pdf-conversion",
        version=VERSION,
        max_file_mb=MAX_FILE_BYTES // 1024 // 1024,
        max_total_mb=MAX_TOTAL_BYTES // 1024 // 1024,
        max_concurrent_jobs=MAX_CONCURRENT_JOBS,
        processing_timeout_seconds=PROCESSING_TIMEOUT_SECONDS,
        conversion_tools=len(SPECS),
        available_conversion_tools=availability,
...
async def analytics_event(event: AnalyticsEvent):
    ...
    if not event.path.startswith("/") or "\n" in event.path or "\r" in event.path:
...
def admin_analytics(
    x_ajn_admin_token: Annotated[str | None, Header()] = None,
    ...
    with sqlite3.connect(ANALYTICS_DB) as db:
        ...
        summary = db.execute(
            """SELECT COUNT(*) AS total,
               ...
               FROM conversion_events WHERE created_at >= datetime('now', ?)""",
            (modifier,),
        ).fetchone()
        tools = db.execute(
            """SELECT tool_id, COUNT(*) AS runs,
               SUM(CASE WHEN status='success' THEN 1 ELSE 0 END) AS success,
               SUM(CASE WHEN status='failed' THEN 1 ELSE 0 END) AS failed,
               AVG(duration_ms) AS avg_duration_ms,
               SUM(input_bytes) AS input_bytes,
               SUM(output_bytes) AS output_bytes
               FROM conversion_events WHERE created_at >= datetime('now', ?)
               GROUP BY tool_id ORDER BY runs DESC LIMIT 100""",
            (modifier,),
        ).fetchall()
        daily = db.execute(
        ...
        site_summary = db.execute(
            """SELECT COUNT(*) AS events,
               SUM(CASE WHEN event_name='page_view' THEN 1 ELSE 0 END) AS page_views,
               SUM(CASE WHEN event_name='tool_open' THEN 1 ELSE 0 END) AS tool_opens,
               SUM(CASE WHEN event_name='tool_start' THEN 1 ELSE 0 END) AS tool_starts,
               SUM(CASE WHEN event_name='tool_complete' THEN 1 ELSE 0 END) AS tool_completes,
               SUM(CASE WHEN event_name='tool_error' THEN 1 ELSE 0 END) AS tool_errors,
               SUM(CASE WHEN event_name='download' THEN 1 ELSE 0 END) AS downloads,
               SUM(CASE WHEN event_name='search' THEN 1 ELSE 0 END) AS searches,
               SUM(CASE WHEN event_name='interaction' THEN 1 ELSE 0 END) AS interactions,
               SUM(CASE WHEN event_name='media_view' THEN 1 ELSE 0 END) AS media_views,
               SUM(CASE WHEN event_name='media_open' THEN 1 ELSE 0 END) AS media_opens
               FROM site_events WHERE created_at >= datetime('now', ?)""",
            (modifier,),
        ).fetchone()
        pages = db.execute(
        ...
        events = db.execute(
            """SELECT event_name, COUNT(*) AS total FROM site_events
               WHERE created_at >= datetime('now', ?)
               GROUP BY event_name ORDER BY total DESC""",
            (modifier,),
        ).fetchall()
        vitals = db.execute(
            """SELECT metric_name, COUNT(*) AS samples, AVG(metric_value) AS average,
               SUM(CASE WHEN metric_rating='good' THEN 1 ELSE 0 END) AS good,
               SUM(CASE WHEN metric_rating='needs-improvement' THEN 1 ELSE 0 END) AS needs_improvement,
               SUM(CASE WHEN metric_rating='poor' THEN 1 ELSE 0 END) AS poor
               FROM site_events WHERE event_name='web_vital'
               AND created_at >= datetime('now', ?)
               GROUP BY metric_name ORDER BY metric_name""",
            (modifier,),
        ).fetchall()
        categories = db.execute(
            """SELECT category, COUNT(*) AS total FROM site_events
               WHERE category IS NOT NULL AND created_at >= datetime('now', ?)
               GROUP BY category ORDER BY total DESC""",
            (modifier,),
        ).fetchall()
        devices = db.execute(
            """SELECT device_type, viewport_bucket, COUNT(*) AS total FROM site_events
               WHERE device_type IS NOT NULL AND created_at >= datetime('now', ?)
               GROUP BY device_type, viewport_bucket ORDER BY total DESC""",
            (modifier,),
        ).fetchall()
        referrers = db.execute(
            """SELECT referrer_group, COUNT(*) AS total FROM site_events
               WHERE referrer_group IS NOT NULL AND created_at >= datetime('now', ?)
               GROUP BY referrer_group ORDER BY total DESC""",
            (modifier,),
        ).fetchall()
        themes = db.execute(
            """SELECT theme, COUNT(*) AS total FROM site_events
               WHERE theme IS NOT NULL AND created_at >= datetime('now', ?)
               GROUP BY theme ORDER BY total DESC""",
            (modifier,),
        ).fetchall()
        connections = db.execute(
            """SELECT connection_type, COUNT(*) AS total FROM site_events
               WHERE connection_type IS NOT NULL AND created_at >= datetime('now', ?)
               GROUP BY connection_type ORDER BY total DESC""",
            (modifier,),
        ).fetchall()
        interactions = db.execute(
            """SELECT element_id, COUNT(*) AS total FROM site_events
               WHERE event_name IN ('interaction','outbound_click')
               AND element_id IS NOT NULL AND created_at >= datetime('now', ?)
               GROUP BY element_id ORDER BY total DESC LIMIT 50""",
            (modifier,),
        ).fetchall()
        search_buckets = db.execute(
            """SELECT query_length_bucket, COUNT(*) AS total FROM site_events
               WHERE event_name='search' AND query_length_bucket IS NOT NULL
               AND created_at >= datetime('now', ?)
               GROUP BY query_length_bucket ORDER BY total DESC""",
            (modifier,),
        ).fetchall()
        site_daily = db.execute(
            """SELECT substr(created_at, 1, 10) AS day,
               SUM(CASE WHEN event_name='page_view' THEN 1 ELSE 0 END) AS page_views,
               SUM(CASE WHEN event_name='tool_start' THEN 1 ELSE 0 END) AS starts,
               SUM(CASE WHEN event_name='tool_complete' THEN 1 ELSE 0 END) AS completes,
               SUM(CASE WHEN event_name='download' THEN 1 ELSE 0 END) AS downloads
               FROM site_events WHERE created_at >= datetime('now', ?)
               GROUP BY day ORDER BY day ASC""",
            (modifier,),
        ...
        realtime = db.execute(
            """SELECT substr(created_at, 1, 13) || ':00' AS hour,
               COUNT(*) AS events,
               SUM(CASE WHEN event_name='page_view' THEN 1 ELSE 0 END) AS page_views,
               SUM(CASE WHEN event_name='tool_complete' THEN 1 ELSE 0 END) AS completes
               FROM site_events WHERE created_at >= datetime('now', '-24 hours')
    ...
    if MEDIA_DB.exists():
        with sqlite3.connect(MEDIA_DB) as media_db:
            ...
            media_row = media_db.execute(
                "SELECT COUNT(*) AS published_posts, MAX(published_at) AS latest_published_at FROM public_posts WHERE published=1"
            ).fetchone()
            if media_row:
    ...
    funnel = {
        "start_to_complete_rate": round((completes / starts * 100), 2) if starts else 0,
        "complete_to_download_rate": round((downloads / completes * 100), 2) if completes else 0,
        "tool_error_rate": round((errors / starts * 100), 2) if starts else 0,
    }
    return {
        ...
        "privacy": {
            ...
            "account_details_stored": False,
            "ip_addresses_stored": False,
            "raw_ip_addresses_persisted": False,
        },
        ...
        "site_daily": [dict(row) for row in site_daily],
        ...
        "search_buckets": [dict(row) for row in search_buckets],
        "realtime": [dict(row) for row in realtime],
    }
...
async def protect_pdf(
    ...
    user_password: Annotated[str, Form(min_length=4, max_length=128)],
    owner_password: Annotated[str, Form(max_length=128)] = "",
    allow_printing: Annotated[bool, Form()] = True,
    allow_copying: Annotated[bool, Form()] = False,
    allow_editing: Annotated[bool, Form()] = False,
    allow_annotations: Annotated[bool, Form()] = False,
    allow_form_filling: Annotated[bool, Form()] = True,
    output_name: Annotated[str, Form(max_length=120)] = "protected",
):
...
async def unlock_pdf(
    ...
    output_name: Annotated[str, Form(max_length=120)] = "unlocked",
):
    if not authorized:
    ...
    try:
        ...
        async with _PROCESSING_SEMAPHORE:
            await asyncio.to_thread(
                _run_worker_job,
                {"operation": "unlock", "source": str(source), "target": str(target), "password": password},
                min(PROCESSING_TIMEOUT_SECONDS, 180),
            )
