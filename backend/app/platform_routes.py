...
async def _run_electronic_signature_package(
    pdf: UploadFile,
    ...
    try:
        ...
        async with main_api._PROCESSING_SEMAPHORE:
            await asyncio.to_thread(
                ...
                min(main_api.PROCESSING_TIMEOUT_SECONDS, 180),
            )
        ...
        headers.update(
            {
                "X-AJN-Evidence-ID": str(evidence_payload.get("evidence_id") or ""),
                "X-AJN-Signature-Type": "electronic-signature",
                "X-AJN-Certificate-Signature": "false",
                "X-AJN-Temporary-Processing": "true",
            }
        )
        return FileResponse(
            package,
            filename=f"{Path(final_name).stem}-package.zip",
    ...
    return JSONResponse(
        {
            ...
            "electronic_signature": {
                ...
                "certificate_backed_pades": False,
            },
        },
        headers=_api_headers(principal, rate),
    )
...
async def api_v1_sign_electronic(
    pdf: Annotated[UploadFile, File(...)],
    signature: Annotated[UploadFile, File(...)],
    signer_name: Annotated[str, Form(min_length=1, max_length=120)],
    signer_email: Annotated[str, Form(min_length=3, max_length=180)],
    reason: Annotated[str, Form(max_length=300)] = "I approve and sign this document.",
    consented: Annotated[bool, Form()] = False,
    page: Annotated[int, Form(ge=1)] = 1,
    x: Annotated[float, Form(ge=0)] = 72,
    y: Annotated[float, Form(ge=0)] = 72,
    width: Annotated[float, Form(gt=0)] = 165,
    height: Annotated[float, Form(gt=0)] = 78,
    output_name: Annotated[str, Form(max_length=120)] = "signed",
    x_ajn_api_key: Annotated[str | None, Header(alias="X-AJN-API-Key")] = None,
):
    principal, rate = await _guard(x_ajn_api_key, "sign")
    return await _run_electronic_signature_package(
        pdf,
        signature,
        signer_name,
        signer_email,
