...
def electronic_sign(
    ...
    embedded_evidence = {
        ...
        "intent": {"consented": True, "consent_text": CONSENT_TEXT, "reason": reason},
        "document": {
            "original_filename": _clean(options.get("original_filename") or source.name, 200),
            "original_sha256": original_sha256,
            "signed_content_sha256": signed_content_sha256,
            "final_pdf_sha256": None,
            "final_hash_note": "The companion evidence JSON contains the hash of the final PDF after this evidence file is embedded.",
        },
        "signature": {
            "source": _clean(options.get("signature_source") or "upload", 20),
            "signature_image_sha256": signature_sha256,
            "coordinate_system": "PDF page points, top-left origin",
            "page": requested_page,
            "x": x,
            "y": y,
            "width": width,
        ...
        "notice": "This is an AJN PDF evidence-backed electronic signature, not a certificate-backed PAdES digital signature.",
    }
    evidence_bytes = json.dumps(embedded_evidence, ensure_ascii=False, indent=2).encode("utf-8")

    with pikepdf.open(interim) as pdf:
        pdf.attachments["ajn-signature-evidence.json"] = pikepdf.AttachedFileSpec(
            pdf,
            evidence_bytes,
            description="AJN PDF electronic signature evidence manifest",
            filename="ajn-signature-evidence.json",
