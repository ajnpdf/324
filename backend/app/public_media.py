...
def _row_to_post(row: sqlite3.Row) -> dict[str, object]:
    ...
    return {
        ...
        'image_url': f"/media/{row['image_filename']}",
        'thumbnail_url': f"/media/{row['thumbnail_filename']}",
        'width': row['width'],
        'height': row['height'],
...
def _save_image(source: Image.Image, slug: str) -> tuple[str, str, int, int, str]:
    ...
    thumb = ImageOps.fit(source, (640, 640), method=Image.Resampling.LANCZOS, centering=(.5, .5))
...
async def create_post(
    ...
    title: Annotated[str, Form(min_length=5, max_length=120)],
    caption: Annotated[str, Form(min_length=60, max_length=1200)],
    alt_text: Annotated[str, Form(min_length=10, max_length=220)],
    tags: Annotated[str, Form(max_length=240)] = '',
    published: Annotated[bool, Form()] = True,
    scheduled_at: Annotated[str, Form(max_length=40)] = '',
    ...
    try:
        with sqlite3.connect(MEDIA_DB) as db:
            cur = db.execute(
                '''INSERT INTO public_posts(slug,title,caption,alt_text,tags_json,image_filename,thumbnail_filename,image_hash,width,height,published,scheduled_at,published_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',
                (slug, clean_title, clean_caption, clean_alt, json.dumps(clean_tags, ensure_ascii=False), image_filename, thumbnail_filename, source_hash or optimized_hash, width, height, 1 if published else 0, schedule, publication_time, now),
            )
            db.commit()
...
def update_post(
    ...
    with sqlite3.connect(MEDIA_DB) as db:
        ...
        db.execute(
            'UPDATE public_posts SET title=?,caption=?,alt_text=?,tags_json=?,published=?,scheduled_at=?,published_at=?,updated_at=? WHERE id=?',
            (clean_title, clean_caption, clean_alt, json.dumps(_clean_tags(tags), ensure_ascii=False), 1 if published else 0, schedule, publication_time, now, post_id),
        )
        db.commit()
        row = db.execute('SELECT * FROM public_posts WHERE id=?', (post_id,)).fetchone()
    return _row_to_post(row)
...
def delete_post(
    post_id: int,
    x_ajn_admin_token: Annotated[str | None, Header()] = None,
    x_ajn_confirm_title: Annotated[str | None, Header()] = None,
):
    _require_admin(x_ajn_admin_token)
    with sqlite3.connect(MEDIA_DB) as db:
        db.row_factory = sqlite3.Row
...
# The main FastAPI app already includes this router. Mounting the platform router
# here adds API v1 and OCR analysis without creating a circular import in main.py.
