...
def _negative_tests(session: requests.Session, root: Path, samples: dict[str, Path]) -> None:
    ...
    response = _post_with_retry(
        session,
        "/api/convert/not-a-real-tool",
        data={"options_json": "{}", "output_name": "bad"},
        sources=[samples[".txt"]],
        expected_status=404,
        request_id="http-accept-unknown-tool",
    )
    ...
    # Invalid JSON options are rejected before conversion.
    response = _post_with_retry(
        session,
        "/api/convert/txt-to-pdf",
        data={"options_json": "[]", "output_name": "bad-options"},
        sources=[samples[".txt"]],
        expected_status=400,
        request_id="http-accept-options",
    )
    ...
    _post_with_retry(
        session,
        "/api/convert/txt-to-pdf",
        data={"options_json": "{}", "output_name": "empty"},
        sources=[empty],
        expected_status=400,
        request_id="http-accept-empty",
    )

    # A renamed non-PDF is rejected by content validation rather than handed to PyMuPDF.
    fake_pdf = root / "fake.pdf"
    fake_pdf.write_text("not really a pdf", encoding="utf-8")
    response = _post_with_retry(
        session,
        "/api/convert/pdf-to-txt",
        data={"options_json": "{}", "output_name": "fake"},
        sources=[fake_pdf],
        expected_status=415,
        request_id="http-accept-fake-pdf",
    )
    if response.json().get("code") != "UNSUPPORTED_FORMAT":
        raise RuntimeError("Invalid PDF content did not return UNSUPPORTED_FORMAT.")

    # Single-file tools reject multiple uploads before processing.
    _post_with_retry(
        session,
        "/api/convert/pdf-to-txt",
        data={"options_json": "{}", "output_name": "too-many"},
        sources=[samples[".pdf"], samples[".pdf"]],
        expected_status=400,
        request_id="http-accept-too-many",
    )

    # Multi-file tools still enforce a global file-count ceiling.
    _post_with_retry(
        session,
        "/api/convert/image-to-pdf",
        data={"options_json": "{}", "output_name": "too-many-images"},
        sources=[samples[".png"]] * 51,
        expected_status=400,
        request_id="http-accept-file-count",
    )


def _security_endpoints(session: requests.Session, root: Path, source_pdf: Path) -> list[str]:
    passed: list[str] = []
    protected_response = _post_with_retry(
        session,
        "/api/pdf/protect",
        data={
            "user_password": "AJN-HTTP-2026",
            "owner_password": "AJN-HTTP-OWNER-2026",
            "allow_printing": "true",
            "allow_copying": "false",
            "allow_editing": "false",
            "allow_annotations": "false",
            "allow_form_filling": "true",
            "output_name": "http-protected",
        },
        sources=[source_pdf],
        field_name="file",
        request_id="http-accept-protect",
    )
    ...
    wrong = _post_with_retry(
        session,
        "/api/pdf/unlock",
        data={"password": "wrong", "authorized": "true", "output_name": "wrong"},
        sources=[protected],
        field_name="file",
        expected_status=401,
        request_id="http-accept-wrong-password",
    )
    if wrong.json().get("code") != "WRONG_PASSWORD":
        raise RuntimeError("Wrong-password response did not return WRONG_PASSWORD.")

    unlocked_response = _post_with_retry(
        session,
        "/api/pdf/unlock",
        data={"password": "AJN-HTTP-2026", "authorized": "true", "output_name": "http-unlocked"},
        sources=[protected],
        field_name="file",
        request_id="http-accept-unlock",
    )
    ...
    repaired_response = _post_with_retry(
        session,
        "/api/pdf/repair",
        data={"output_name": "http-repaired"},
        sources=[source_pdf],
        field_name="file",
        request_id="http-accept-repair",
    )
    ...
    compressed_response = _post_with_retry(
        session,
        "/api/pdf/compress",
        data={"output_name": "http-compressed"},
        sources=[source_pdf],
        field_name="file",
        request_id="http-accept-compress",
    )
...
def main() -> None:
    with warnings.catch_warnings():
        warnings.filterwarnings("ignore", message=r"In the future version we will turn default option ignore_ncx to True\.")
        warnings.filterwarnings("ignore", message=r"This search incorrectly ignores the root element.*")
        with tempfile.TemporaryDirectory(prefix="ajn-http-acceptance-") as directory:
            ...
            for tool_id, spec in sorted(SPECS.items()):
                ...
                try:
                    response = _post_with_retry(
                        session,
                        f"/api/convert/{tool_id}",
                        data={
                            "options_json": json.dumps({"language": "eng", "dpi": 96, "quality": 70, "grayscale": False}),
                            "output_name": f"http-{tool_id}",
                            "source_url": "",
                        },
                        sources=sources,
                        request_id=_request_id(tool_id),
                    )
            ...
            report = {
                "version": "3.1.0",
                "generic_endpoint_passed": passed,
                "security_endpoint_passed": security_passed,
                "skipped": skipped,
                "failed": failed,
            }
            ...
            print(
                "PASS: live HTTP acceptance verified "
                f"{len(passed)} generic conversion endpoints + {len(security_passed)} PDF service endpoints; "
                f"{len(skipped)} dependency/deployment fixture cases documented."
            )
