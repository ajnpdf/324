...
def post(path: str, fields: dict[str, str], source: Path | None = None, expected_status: int = 200, field_name: str = "file", extra_headers: dict[str, str] | None = None) -> bytes:
    ...
    request = urllib.request.Request(
        ...
        data=body,
        method="POST",
        headers={"Content-Type": f"multipart/form-data; boundary={boundary}", "X-Request-ID": "smoke-test", **(extra_headers or {})},
    )
    try:
...
def post_json(path: str, payload: dict[str, object], expected_status: int = 204) -> bytes:
    request = urllib.request.Request(
        ...
        method="POST",
        headers={"Content-Type": "application/json", "X-Request-ID": "analytics-smoke-test"},
    )
    try:
        with urllib.request.urlopen(request, timeout=20) as response:
            status = response.status
...
def main() -> None:
    ...
    assert any(tool["id"] == "scanned-pdf-to-text" for tool in tools)
    ...
    analytics_request = urllib.request.Request(
        f"{BASE_URL}/api/admin/analytics?window_days=30",
        headers={"X-AJN-Admin-Token": analytics_token()},
    )
    ...
    with tempfile.TemporaryDirectory(prefix="ajn-smoke-") as directory:
        ...
        media_payload = json.loads(post(
            "/api/admin/posts",
            {
                "title": "AJN PDF public media smoke test",
                "caption": "This original AJN PDF test image verifies public publishing, optimized media delivery, descriptive content and protected admin deletion.",
                "alt_text": "White AJN PDF smoke test image used during automated media publishing verification",
                "tags": "AJN PDF, smoke test, public media",
                "published": "true",
            ...
            expected_status=201,
            field_name="image",
            extra_headers={"X-AJN-Admin-Token": media_token()},
        ))
        ...
        deleted = json.loads(delete(
            f"/api/admin/posts/{media_payload['id']}",
            {"X-AJN-Admin-Token": media_token(), "X-AJN-Confirm-Title": "AJN PDF public media smoke test"},
        ))
        assert deleted["deleted"] is True
        image_pdf = post(
            "/api/convert/png-to-pdf",
            {"options_json": "{}", "output_name": "image-result"},
            image,
            field_name="files",
        )
        ...
        text_pdf = post(
            ...
            {"options_json": "{}", "output_name": "text-result"},
            text_file,
            field_name="files",
        )
        ...
        pdf_text = post(
            ...
            {"options_json": "{}", "output_name": "pdf-text"},
            source,
            field_name="files",
        )
        ...
        png_zip = post(
            ...
            {"options_json": '{"dpi":96}', "output_name": "pages"},
            source,
            field_name="files",
        )
        assert png_zip.startswith(b"PK")

    print("PASS: backend security, analytics privacy, CRO funnel, public media, OCR registry and core conversion smoke tests")
