...
def _docx_fixture(path: Path, image_path: Path) -> None:
    ...
    for r, values in enumerate([['Product','Count','Amount'],['AJN PDF','12','480'],['OCR','6','240']]):
...
def _pdf_fixture(path: Path) -> None:
    ...
    widths=[180,120,120]
    row_h=34
    rows=[['Product','Count','Amount'],['AJN PDF','12','480'],['OCR','6','240']]
    xs=[x0]
    for w in widths:
        xs.append(xs[-1]+w)
...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix='ajn-r19-fidelity-') as temporary:
        ...
        editable_text='\n'.join(shape.text for slide in eprs.slides for shape in slide.shapes if hasattr(shape,'text'))
