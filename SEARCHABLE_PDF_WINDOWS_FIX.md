...
- Replaced temporary-file PDF output with Tesseract binary PDF streaming through `stdout`.
- This avoids Windows builds that return success but do not leave the requested output-base PDF on disk.
- Tesseract still creates the multilingual searchable layer with its embedded GlyphLessFont.
- Each streamed page is validated as a one-page PDF before being saved and merged.
- PyMuPDF performs deterministic multi-page merging and the acceptance suite verifies the OCR text layer.
