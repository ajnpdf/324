...
- stops sending unnecessary multipart `options_json` JSON in OCR/DOCX requests
- relies on backend defaults (`options_json={}`, OCR language `eng`)
- captures HTTP status and server error body instead of hiding it behind curl `-f`
- uses `${Label}`-safe interpolation via `$($Label)`
- generates a large high-contrast OCR fixture
- validates recognized OCR content
